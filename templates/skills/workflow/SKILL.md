...
- `execution_mode` 只定义语义暂停偏好，不单独主导继续/停顿决策
- `ContextGovernor` 先判断 projected budget、验证门控与治理边界，再决定是否继续
- 当同阶段存在 2+ 可证明独立边界、且规划工件已稳定时，`parallel-boundaries` 是首选的上下文减压策略之一
- phase 应服务于治理边界，而不是微实现步骤切分
...
质量关卡任务的 `quality_review` action 内部包含两阶段审查，并作为 shared review loop contract 的 execution adapter：planning side 写入 `review_status.*`，execution side 写入 `quality_gates.*`，详见 `specs/execute/actions/quality-review.md`。
...
- 当主会话已进入上下文 warning 区且规划工件稳定时，边界并行优先于主会话顺序吞下多个独立任务
...
- **Phase 1.2: Spec Review** — `machine_loop`：检查设计是否已经达到可写规范文档的质量线，并验证 traceability completeness / critical constraint preservation / scope decision explicitness
- **Phase 1.3: Spec Generation** — 生成用户友好的 `spec.md`
- **Phase 1.4: User Spec Review** — `human_gate`：用户确认范围、模块和验收映射
- **Phase 1.5: Intent Review** — `conditional_human_gate`：先做机器一致性检查，再按风险决定是否进入人工确认
- **Phase 2: Plan Generation** — 从 `spec + baseline + brief` 生成 `plan.md`
- **Phase 2.5: Plan Review** — `machine_loop`：检查计划完整性、粒度、可执行性与 requirement coverage
...
/workflow execute                         # 执行下一个任务（默认治理 phase 模式）
