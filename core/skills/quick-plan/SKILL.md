---
name: quick-plan
description: "轻量快速规划 - 适用于简单到中等任务,直接产出可执行的 plan.md,不走 workflow 状态机。触发条件:用户说「快速规划」「轻规划」「不走workflow」「plan 一下」「quick plan」,或需求清晰、作用域明确、可一次性规划完成。复杂项目(跨module / 新子系统 / 需追溯)请使用 /workflow-plan。"
...
> - 要修改 plan → 告诉我反馈，我调整
> - 要升级到完整workflow（spec + 状态机 + 追溯）→ `/workflow-plan`
...
| `/workflow-plan` | 复杂 / 跨module，需 spec 追溯 | `spec.md` + `plan.md` + 状态机 |

- `/quick-plan` 只生成轻量 `plan.md`，不进入 workflow 状态机
- `/quick-plan` 不触发 UX 设计审批、需求讨论等 HARD-GATE
- 过程中发现任务复杂度升到 XL 级 → 切换到 `/workflow-plan`
- 接受 plan 后想按 workflow 执行 → 建议先 `/workflow-plan` 升级为完整workflow（含 spec + 状态机）。直接 `/workflow-execute` 会因缺少 spec 而要求确认降级
