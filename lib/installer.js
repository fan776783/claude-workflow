      ...
      if (global) {
        try {
          ...
          if (options.notify !== false) {
            agentResult.notifyHooks = await ensureNotifyHooks(settingsPath, hooksDir);
          } else {
            agentResult.notifyHooks = {
              injected: false,
              events: [],
              skipped: ['notify hooks disabled via options.notify=false'],
            };
          }
        } catch (err) {
          agentResult.workflowHooks = { injected: false, error: err.message };
          agentResult.teamHooks = { injected: false, error: err.message };
          agentResult.notifyHooks = { injected: false, error: err.message };
        }
      } else {
        ...
        agentResult.notifyHooks = {
          injected: false,
          events: [],
          skipped: ['project-level install skips notify hook injection'],
        };
      }
    }
...
const NOTIFY_HOOK_DEFS = {
  Stop: {
    script: 'notify.js',
    args: ['Stop'],
    description: '任务完成时弹跨平台桌面通知',
  },
  Notification: {
    script: 'notify.js',
    args: ['Notification'],
    description: '等待输入 / 通知事件时弹跨平台桌面通知',
  },
};
...
/**
 * 清理用户 settings.json 里遗留的 notify.sh 条目（shell 版本），
 * 避免 notify.sh 与 notify.js 并存导致每个事件响两次通知。
 * 只在 Stop / Notification 两个事件下扫，只移除 command 包含 "notify.sh" 的条目。
 */
async function sweepLegacyNotifyShell(settingsPath) {
  ...
  const targetEvents = ['Stop', 'Notification'];
  let modified = false;
  ...
  for (const eventName of targetEvents) {
    ...
    const cleaned = entries.filter((entry) => {
      ...
      // 移除任何 command 中包含 notify.sh 的条目（覆盖 ~/.claude/notify.sh、./notify.sh 等）
      return !hooks.some((h) => typeof h?.command === 'string' && /\bnotify\.sh\b/.test(h.command));
    });
  ...
  if (modified) {
    await fs.ensureDir(path.dirname(settingsPath));
    await fs.writeJson(settingsPath, settings, { spaces: 2 });
    console.log(`${LOG_PREFIX} 已迁移 notify.sh → notify.js (清理了遗留 Stop/Notification 条目)`);
  }

  return modified;
}

async function ensureNotifyHooks(settingsPath, hooksDir) {
  await sweepLegacyNotifyShell(settingsPath);
  const result = await ensureManagedHooks(settingsPath, hooksDir, NOTIFY_HOOK_DEFS);
  if (result.injected) {
    console.log(`${LOG_PREFIX} 已注入 notify hooks: ${result.events.join(', ')}`);
  }
  return result;
}
...
async function ensureWorkflowHooks(settingsPath, hooksDir) {
  ...
  // 清理已废弃的 hook：
  //   - PostToolUse quality-gate-loop（v13.1 移除）
  //   - WorktreeCreate worktree-serialize / WorktreeRemove worktree-cleanup（上游并发问题已缓解）
  const deprecatedByEvent = {
    PostToolUse: ['quality-gate-loop.js'],
    WorktreeCreate: ['worktree-serialize.js'],
    WorktreeRemove: ['worktree-cleanup.js'],
  };
  ...
  if (await fs.pathExists(settingsPath)) {
    try {
      ...
      for (const [eventName, deprecated] of Object.entries(deprecatedByEvent)) {
        const entries = settings.hooks?.[eventName];
        if (!Array.isArray(entries)) continue;
        ...
        if (cleaned.length !== entries.length) {
          ...
          modified = true;
          result.events.push(`${eventName}(removed deprecated)`);
        }
      }
  ...
  if (await fs.pathExists(canonicalDir) || (status.mode === INSTALL_MODE_REPO_LINK && await fs.pathExists(status.sourceRoot))) {
    ...
    for (const [name, config] of Object.entries(agents)) {
      ...
      if (name === 'claude-code') {
        ...
        agentStatus.notifyHooks = await inspectManagedHooks(settingsPath, hooksDir, NOTIFY_HOOK_DEFS);
