...
> CLI 已内置识别逻辑。若用户意图不明确（例如参数无法判定类型、同时满足多种模式），调用 `AskUserQuestion` 收集变更类型，`question` 写"本次变更属于哪种类型？"，`options` 给 4 条：
>
> - `sync` — API 同步（执行项目配置的 API 同步命令，自动解除 `api_spec` 阻塞）
> - `api` — API 规格变更（分析接口差异，生成任务变更）
> - `prd` — PRD 文件更新（对比新旧需求）
...
展示变更摘要后，调用 `AskUserQuestion` 收集决策，`question` 写"如何处理本次变更？"，`options` 给三条：

- `apply` — 应用变更：进入 Step 6（占位路径必须先替换）
- `manual_edit` — 手动编辑：暂停，用户编辑 `intent.md` 后重新执行
- `cancel` — 取消：调用 `delta fail` 标记变更失败，退出
