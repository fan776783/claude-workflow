---
name: figma-ui
description: "Use when 用户提供 Figma URL(figma.com/design|file|proto) 要求实现/还原/构建 Web UI; or 引用现有文件说「按设计稿还原/调整/检查」「和设计不一致」「还原度不够」; or 要求把 icon/empty-state/插画资源换成设计稿里的; or 用户贴了 Figma 截图要求照着写; or 说「还原设计稿」「按设计稿写」「照着 Figma 写」「换个 icon」「这个组件和设计对不上」「convert this design to code」「restore the design」「implement this design」「match the mockup」。即使用户没明确说 Figma，只要上下文涉及设计稿→Web 代码还原就应触发。Figma 画布操作(create/edit/delete nodes)请用 figma-use skill。"
---
