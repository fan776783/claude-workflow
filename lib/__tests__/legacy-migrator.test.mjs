...
// macOS/Linux 上 os.homedir() 读 $HOME；Windows 上优先读 USERPROFILE，
// 其次 HOMEDRIVE+HOMEPATH。统一 swap 全部三个 + HOME 以保证跨平台。
async function withTempHome(fn) {
  ...
  // 把 HOMEDRIVE/HOMEPATH 拆出来（Windows 拼接逻辑）
  if (process.platform === 'win32' && /^[A-Za-z]:/.test(tmp)) {
    process.env.HOMEDRIVE = tmp.slice(0, 2)
    process.env.HOMEPATH = tmp.slice(2)
  } else {
