...
5. ☐ 设计深化(自动路由,≥3 module 或 ≥4 page 时触发)+ Self-Review
6. ☐ 🛑 用户审批 + 规划完成
...
> § 4.4 / § 5.6 的一致性检查由 `/ux-elaboration` 和 `/system-design` 各自内部的 Self-Review 覆盖,本 Step 不涉及。
...
根据上下文信号直接决策深化范围，**不使用 AskUserQuestion**。执行前输出一行声明（如 `→ 执行前端 + 后端设计深化`），用户可在 Step 6 审批时要求追加或删减。
...
| `ux_gate_required=true` 且 § 5.1 无后端服务模块 | frontend only |
| `ux_gate_required=false` 且 § 5.1 含 API/Service/DB 层 | backend only |
...
2. 设计深化摘要(§ 4.4 / § 5.6,如有)+ 自动路由决策声明（用户可在此追加/删减/重做）
3. PRD 覆盖率(即时计算)
4. Codex review 发现(若已执行)

**设计深化覆盖**:用户在此步可要求:
- 追加未执行的深化（如自动路由只做了 frontend，用户要求补 backend）
- 删减已执行的深化（删除 § 4.4 或 § 5.6 内容）
- 重做某一深化（如对 § 5.6 API 设计不满意）

覆盖后回到 Step 5 按用户指令重新执行对应 skill。
