...
const DIRECT_LINK_DIRS = ['commands', 'prompts', 'utils', 'specs'];
...
async function installBinary(packageDir) {
  ...
  try {
    ...
    try {
      ...
      console.log(`${LOG_PREFIX} 二进制验证失败: ${verifyError.message}`);
      return { success: false, reason: 'verification_failed', error: verifyError.message, installDir };
    }

    console.log(`${LOG_PREFIX} 已安装 codeagent-wrapper 到: ${destPath}`);

    // 检查是否在 PATH 中
    const inPath = isInPath(installDir);
    if (!inPath) {
      console.log(`${LOG_PREFIX} 警告: ${installDir} 不在 PATH 中`);
    }

    return { success: true, path: destPath, installDir, inPath };
  } catch (err) {
    console.log(`${LOG_PREFIX} 安装二进制失败: ${err.message}`);
