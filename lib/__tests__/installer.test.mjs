...
test('installForAgents: codebuddy 落出 skills(顶层)/commands(顶层 .md)/agents(subagents) + hooks 注入 settings.json', async () => {
  await withTempDir(async (tmp) => {
    const result = await installer.installForAgents({
      templatesDir: REPO_ROOT,
      agents: ['codebuddy'],
    ...
    assert.equal(result.errors.length, 0, `errors: ${JSON.stringify(result.errors)}`)
    const cbBase = path.join(tmp, '.codebuddy')

    // skills：~/.codebuddy/skills/<name>/SKILL.md（顶层 skill 目录，非命名空间）
    const skillEntries = (await fs.readdir(path.join(cbBase, 'skills'))).filter((n) => !n.startsWith('.'))
    assert.ok(skillEntries.length > 0, 'codebuddy skills 目录为空')
    await fs.stat(path.join(cbBase, 'skills', skillEntries[0], 'SKILL.md')) // throws if symlink 不可达

    // commands：~/.codebuddy/commands/*.md（顶层 .md）
    const cmdEntries = (await fs.readdir(path.join(cbBase, 'commands'))).filter((n) => n.endsWith('.md'))
    assert.ok(cmdEntries.length > 0, 'codebuddy commands 顶层无 .md')

    // subagents：~/.codebuddy/agents/*.md（net-new mount，按 agentsDir 门控）
    const agentEntries = (await fs.readdir(path.join(cbBase, 'agents'))).filter((n) => n.endsWith('.md'))
    assert.ok(agentEntries.length > 0, 'codebuddy agents 目录无 subagent .md')
    assert.ok(
      await fs.stat(path.join(cbBase, 'agents', '.agent-workflow-managed-agents.json')).then(() => true).catch(() => false),
      'codebuddy agents manifest 未写出',
    )

    // managed namespace：.agent-workflow/{hooks,specs,utils}
    for (const sub of ['hooks', 'specs', 'utils']) {
      const stat = await fs.lstat(path.join(cbBase, '.agent-workflow', sub))
      assert.ok(stat.isSymbolicLink() || stat.isDirectory(), `${sub} 应存在`)
    }

    // hooks 注入 ~/.codebuddy/settings.json（非独立 hooks.json）
    const settingsRaw = await fs.readFile(path.join(cbBase, 'settings.json'), 'utf8')
    const settings = JSON.parse(settingsRaw)
    assert.ok(settings.hooks.SessionStart, 'settings.json 缺 SessionStart')
    assert.ok(settings.hooks.PreToolUse, 'settings.json 缺 PreToolUse')
    assert.equal(/\{\{HOOKS_DIR\}\}/.test(settingsRaw), false, 'HOOKS_DIR 占位符未替换')
    assert.equal('version' in settings, false, 'settings.json 被注入了多余的 version 顶层键')
    const cmd = settings.hooks.SessionStart[0].hooks[0].command
    assert.ok(cmd.includes(path.join(cbBase, '.agent-workflow', 'hooks')), 'hooks 命令未指向 managedDir/hooks')
  })
})

test('agents 配置：codebuddy 声明 installer-mount 字段（skillsDir/agentsDir/hooksFile→settings.json，memory→CODEBUDDY.md）', () => {
  const c = agents.codebuddy
  assert.equal(c.managedViaPlugin, undefined, 'codebuddy 走 installer-mount，不应是 managedViaPlugin')
  assert.match(c.skillsDir, /skills$/)
  assert.match(c.agentsDir, /agents$/)
  assert.match(c.globalHooksFile, /settings\.json$/, 'codebuddy hooks 应注入 settings.json')
  assert.equal(c.hooksTemplate, 'codebuddy.hooks.json')
  assert.equal(c.globalMemory.source, 'AGENTS.md', 'codebuddy memory 源应为 AGENTS.md')
  assert.match(c.globalMemory.globalDest, /CODEBUDDY\.md$/, 'codebuddy memory 应写到 CODEBUDDY.md')
})
