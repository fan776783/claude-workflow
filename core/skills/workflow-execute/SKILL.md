...
**管线完整性规则**：步骤 ①→④ 为强制项，不得跳过或合并。步骤 ② 的检查内容是建议性的，但**输出证据是强制的**（见下方 ② 说明）。连续执行多个 task 时，每个 task 独立走完管线后才进入下一个，不得攒到最后批量执行。

> ⚠️ **批量化检测**：如果你发现自己在最后一个 task 完成后才一次性更新 plan.md 和 state.json，说明你违反了管线规则。正确做法是每完成一个 task 就立即输出 checkpoint 行（见步骤 ③④），然后再开始下一个 task。
...
| ⑤ | **Journal（条件）** | 暂停/完成时调用 `node workflow_cli.js journal add` |
...
示例：
```
✅ T1 checkpoint：plan.md ✓ state.json ✓（completed: [T1], current: [T2]）
✅ T2 checkpoint：plan.md ✓ state.json ✓（completed: [T1,T2], current: [T3]）
```

> ⚠️ 如果你无法输出此行，说明你没有实际更新 plan.md 或 state.json——请立即补做。
...
> **最小化要求**：连续模式下不要求每个 task 前都调用 ContextGovernor，但**至少在最后一个 task 前**必须执行一次 `decide` 调用，确认 budget 和治理状态允许继续。
