...
- `schemaVersion`（必须 = `"1.0"`，见下方 Gate 0）
...
**Gate 0 → schemaVersion assert（ADR-0001 Decision 6）**: 读取 Design Package 后**先校验** `schemaVersion === "1.0"`。
- 不存在该字段或值不匹配 → figma-data 输出过期或 server 漂移；要求用户更新 `@justinfan/agent-workflow` 后重跑，**停止 workflow**。
- 不要"宽容"地继续——schema mismatch 意味下游 contract 失效，强行解析会静默错位。

**Gate → Phase B**: Design Package 就绪（schemaVersion 正确 + AssetPlan 完成且无 `refetch-parent`）。

**Abort → Exit**: figma-data 返回降级产出（`mode: "read-only-fallback"`，无 designContext / 无 AssetPlan）时：
- 将截图 + 节点结构（metadata）呈现给用户
- 告知 Phase B 无法启动（缺少 designContext + AssetPlan）
- 降级原因从 `reason` 字段读取，常见：
  - `get_design_context not available on server (tool_not_found)` → Figma MCP 升级删/改了 tool。`figma-data` 已在 stderr 输出 `{"kind":"tool_not_found",...}`；用户需更新 figma-data 或检查 Figma Desktop MCP 版本
  - `Cannot write to this directory` → ① Figma Desktop → Preferences → Dev Mode MCP → Allowed directories 添加项目路径；② 或切换 Remote MCP（`claude mcp add figma-mcp --transport http --url https://mcp.figma.com/mcp`）
- **停止 workflow，不继续编码**
...
- [`references/playbook.md`](references/playbook.md) — Phase B/C 详细执行 workflow
