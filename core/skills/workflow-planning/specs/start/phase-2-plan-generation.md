...
- `prd-spec-coverage.json`（PRD 覆盖率报告；提供 PRD 段落覆盖状态，用于 coverage drift check）
...
- **Requirement Coverage First** — 先确认 PRD 覆盖率报告 / spec traceability，再拆解文件和步骤
...
- **Protected Nuances Stay Visible** — PRD 覆盖率报告中标注的精确值、否定约束、联动关系、改造指令必须在 Requirement Coverage、任务步骤或验证命令中找到落点
...
const prdCoverage = readPRDCoverageReport(workflowDir);
const requirementCoverage = deriveRequirementCoverage(specContent, prdCoverage, slices);
```
...
for (const slice of slices) {
  for (const file of slice.files) {
    tasks.push({
      ...
      critical_constraints: deriveCriticalConstraintsForSlice(slice, prdCoverage),
...
### Step 4.6: PRD Coverage Drift Check

```typescript
// PRD 覆盖率报告中的 partial/uncovered 段落在 plan task 中是否有落点
...
if (fileExists(coveragePath)) {
  ...
  for (const gap of gaps) {
    const coveredInPlan = tasks.some(task =>
      gap.matchedSpecSections.some(section => task.spec_ref?.includes(section))
    );
    if (!coveredInPlan) {
      console.warn(`⚠️ PRD 段落 [${gap.segmentId}] 未在 Plan 中找到对应 task："${gap.excerpt?.substring(0, 60)}..."`);
    }
  }
}
```
...
function selfReviewPlan(planContent: string, specContent: string): void {
  // 1. Requirement coverage: 逐条检查 PRD 覆盖率报告 / spec 的每个需求
  ...
  // 1.5 PRD Coverage drift: 检查 prd-spec-coverage.json 中 partial/uncovered 段落在 plan 中的落点
  // 已在 Step 4.6 中执行，此处仅记录状态
