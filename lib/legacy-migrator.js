...
class MigrationReport {
  ...
  /**
   * 添加详情记录
   * @param {string} message 详情信息
   * @param {object} context 详情上下文
   */
  ...
  /**
   * 标记迁移完成
   * @param {string} status 最终状态（MigrationStatus 枚举值）
   */
  ...
  /**
   * 将迁移报告转换为 JSON 格式
   * @returns {object} 包含摘要、错误、警告和详情的报告对象
   */
  toJSON() {
    return {
...
class LegacyMigrator {
   ...
   * @param {boolean} options.dryRun 是否为试运行模式（不实际写入）
   * @param {string} options.backupDir 备份目录路径
   */
  constructor(options = {}) {
