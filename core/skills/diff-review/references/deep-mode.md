...
Codex 协作review。不是把 Codex 意见直接展示给用户，而是将 Codex 与当前模型的候选问题统一纳入 adjudication pipeline，完成验证、影响分析和最终裁决。

## 角色

**代码review协调员**，编排双来源候选问题：
...
### Layer A: review范围确认 + Diff Acquisition
...
如遇全栈文件或边界不清晰文件，允许同时纳入两个视角review。

### Layer C-H: 共享review管线

从候选问题发现到最终报告的全部workflow（Candidate Discovery → Normalization → Verification → Impact Analysis → Severity Calibration → Report Synthesis）已统一抽出到 [`../specs/review-pipeline.md`](../specs/review-pipeline.md)。

**调用convention**：Layer A/B 完成后，把delta集（来源描述 + 文件清单 + 统计）交给共享管线。Codex prompt 里必须显式把review范围限定到本次 diff，避免误审。
...
Review Loop 的完整contract见 `../specs/review-pipeline.md`。Deep 模式在此基础上默认停在报告阶段，不自动转入修复；用户显式输入 `fix` 后才进入修复循环。
