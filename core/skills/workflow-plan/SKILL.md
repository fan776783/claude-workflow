...
当 task 描述含 N≥5 个并列子项（筛选项 / 列 / 字段 / 标签 / tab）时，必须拆为 N 个 sub-task，每个 sub-task 带独立 acceptance bullet。**理由**：rm-agent 失败案例 T48 一个 task 覆盖 8 个筛选项漏 1 个 — 多子项聚合 task 漏项不可追溯。`plan-review` 返回 `lints.atomicity.warnings` 标记疑似不达标的 task（advisory 不挡 ready），由 skill 提示用户。阈值 5 可由 spec §3 显式 override。
