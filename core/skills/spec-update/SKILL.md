---
name: spec-update
description: "显式捕获学到的内容并写入 .claude/code-specs/。触发条件：用户调用 /spec-update，或完成一次有沉淀价值的实现/调试/决策后。按 7 段 code-spec 合约或 thinking guide 形态交互式写入。"
---
...
新增 `{pkg}/{layer}/*.md` 时基于 `core/specs/spec-templates/code-spec-template.md` 渲染，每段必填：
...
任一段留为占位符或抽象描述 → `/spec-review` 的 7 段 lint 会标记为未完成。
...
6. **段落标注**：新增或追加的段落首行追加一行元数据：
   ...
   不改 7 段 lint（`/spec-review` 不依赖此标注），仅用于阅读时快速识别语义。
