...
>
> 两类产物分工:机器 task 源(task-dir 元数据 + per-task `context.jsonl` 背包)由**本 skill 现写**——经 CLI `task-write --from-file` / `context-curate --id --from-file` 两条命令(字段 schema + 命令用法见 [`../../specs/workflow-runtime/task-dir-schema.md`](../../specs/workflow-runtime/task-dir-schema.md)),按 implementation slice 定粒度;可选的人类可读 plan.md 叙述骨架(front matter + 锚点)由 `/workflow-spec` Step 6 `spec-review` 生成,本 skill 仅在其上 Edit 扩写、不重建。
...
- task 粒度/ID 在本阶段现写定型(spec-approve 只落了元数据壳);`task-write` 落盘后自动重导 `current_tasks[0]`,无需也不得手工修 state(异常修复用 `repair-anchor`)
...
- `task-write` 整集落盘后自动把 `current_tasks[0]` 重导到新 task 源(C-1 resume 起点不可断);残留孤儿由 `plan-review` 的 `current_tasks_orphaned` lint 兜底挡 ready。锚点损坏时单独修锚走 reseed-only 的 `repair-anchor`(不重写 task 集);完整语义见 workflow-cli.md § task-write 的 resume 锚点重导 / § repair-anchor。
- **plan.md 退化为可选的人类可读叙述**:front matter + 结构锚点 + 可选叙述。它**不再是结构化机器 task 源**——execute 期 `execution_sequencer` 从 task-dir 读 task 序列,不解析 plan.md task block。叙述 plan.md 仅供人 review,可省。
...
- **WorkflowTaskV2 Compatible** — task 以 task.json v2 记录经 `task-write` 落盘，rich 字段齐全（`files`/`constraints`/`patterns`/`mandatory_reading`/`task_text`），非 plan.md markdown block
...
task 含 N≥5 个并列子项（筛选项 / 列 / 字段 / 标签 / tab）→ 拆 N 个 sub-task 各带独立 acceptance bullet（多子项聚合 task 漏项不可追溯）。`plan-review` 的 `lints.atomicity.warnings` 自动标记（advisory 不挡 ready），命中时提示用户确认是否拆分。

### 共享文件信号（Merge / Fan-out）

`plan-review` 的 `lints.shared_file` 自动扫 `files[]`×`depends[]` 图（advisory 不挡 ready），两类信号：

- **`merge_candidates`**：两 task 满足 ①直接 `depends` 边 ∩ ②`files[]` 有交集 ∩ ③同 `phase` ∩ ⑤同 `quality_gate` → 建议合并为一个 task（拆开只多一次 subagent 往返、不增可追溯）。④同功能域机器判不了，**只报候选、人确认后合，引擎绝不自动合并**；合并时逐半 acceptance bullet 挂回原 `R-id/CHG-id` 保留可追溯。
- **`fan_out`**：同一 file 被 ≥3 个 task 触及 → 确认写入顺序/归属；若为**生成物**（下游 i18n locale / autogen 等），feature task 只碰源、物化收敛到单一末尾 task。

与 Task Atomicity 正交：atomicity 看**广度**（一个 task N≥5 并列子项 → 拆），merge 看**深度**（两 task 同产物 + 依赖边 → 合），数据源不同（name/task_text 正则 vs files/depends）不双报。
...
### Global Constraints 块（参照 superpowers 6.0）

如果 spec 中存在约束**每个 task** 的规则（版本下限、依赖限制、命名和文案约定、精确值），把它们写入第一个 task（或所有 task）的 `constraints_global` 字段（string[]）。execute 期 `pre-execute-inject` hook 会逐字注入每个 implementer/reviewer 的 `<global-constraints>` 段。

- **逐字复制到下游**，不摘要、不转述——这些约束到达 implementer 时必须与 spec 原文一致
- 只写真正全局的约束（影响 ≥2 个 task）；单 task 约束仍走 `constraints` 字段
- 空 `constraints_global` = 无全局约束，行为不变（向后兼容）

### Per-Task Interfaces 块（参照 superpowers 6.0）

每个 task 在 `task-write` JSON 的 `interfaces` 字段声明其消费和生产的接口：
...
- 让只看自己 task 的 implementer 仍知道邻居的 contract——T2 不用读 T1 的源码就知道 `UserService.list` 的返回形态
- `from_task` 标注接口来源 task（跨 task contract 追溯）
- 空时 reviewer 不校验接口一致性（向后兼容）

### Right-Sizing 指导（参照 superpowers 6.0）

任务保持在「值得自己的测试周期和审阅器通过」的规模：
- **不要拆太碎**：setup / config / docs 折叠到需要它们的实现 task 中，不单独成 task（省一次 subagent 往返 + reviewer 往返）
- **不要合太大**：一个 task 超过 2-5 分钟原子操作时拆分——否则 reviewer 难以构造有效 refute，review 变成形式主义
- **测试结果**（superpowers 6.0 实测）：right-sized 计划需 1 轮修复，过细/过粗计划需 2-4 轮，且过粗计划发布了真实 bug
- `plan-review` 的 `lints.atomicity`（N≥5 子项必拆）和 `lints.shared_file.merge_candidates`（同产物+依赖边建议合）已自动标记，命中时人工确认

### Confidence Score

`plan-review` 自动算 confidence（score / level / breakdown,rubric 为引擎内部细节,不要逆向）。偏低时读 `confidence.hints` 逐维提升,Step 3 直接 paste `summary.confidence`;`confidence=low` 时摘要标注「建议 review」。

### Discussion Drift Check

Plan 阶段以审批后的 Spec Architecture / Scope 为准,**不得基于 § 9 讨论记录发明 spec 中不存在的任务**;发现 spec 自身矛盾(如 § 9.2 已选方案未反映进 Architecture——该一致性 spec Self-Review 已查过,此处不重扫)一律回 `/workflow-spec` 修订。唯一需主动核对项:spec.md § 9.3 存在未解决依赖时,验证 Spec Scope 对应需求已标记 `blocked`,缺失 → 提示用户回 `/workflow-spec` 修订。
