...
| 对话一开始就被 `/clear` 清空 | 当前上下文从一个几乎空白的起点展开，用户却要求"review本会话的改动" |
...
## Step 3: 展示delta集并直接进入review

默认行为拆成两个动作：**展示清单（必做）** + **立即进入review（不等待回复）**。展示不是可跳过的装饰步骤——用户需要在看到清单后有机会插话调整；省掉展示就等于剥夺了这次机会。
...
把 Step 2 的三项作为**delta集**输入 `../specs/review-pipeline.md`。管线要求的输入contract已经满足：

- delta集来源 = "本会话上下文提取的 N 个文件"
- 文件清单 = Step 2.2 的列表
