...
> 🔧 **自愈例外**：会话丢失重建 state 时，CLI `init` 按 spec 文件存在性推断审批状态（`system-recovery` 标记，非用户主权审批）。详见 `workflow-execute` SKILL.md Step 2。
...
4. **projectId 获取** — 从 `project-config.json` 的 `project.id` 读取，或调用 `stableProjectId()` CLI helper（见 preflight.md Step 2）。禁止 shell 手动哈希（路径规范化差异会不一致）。
...
| 选项 | 快捷回复 | 结果 |
|------|---------|------|
| ✅ Spec 正确，生成 Plan | `ok` / `通过` / `LGTM` / `approve` / `没问题` / `可以` | 进入 Plan 生成 |
| 🔄 需要修改 Spec | 以 `修改` / `改` 开头，或直接描述修改点 | 回到 Step 5 |
| ❌ 需要拆分范围 | `拆分` / `太大` / `范围太广` | 状态回 idle，缩小范围后重启 |
