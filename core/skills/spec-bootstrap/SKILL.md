---
name: spec-bootstrap
description: "初始化项目级 code-specs 目录骨架。触发条件：用户调用 /spec-bootstrap，或 /scan 在检测到未初始化时引导调用。根据 project-config.json 的 monorepo.packages 与 tech.frameworks 生成 {pkg}/{layer}/index.md 与 guides/index.md，并创建 local.md 记录模板基线。"
---

# /spec-bootstrap

在项目中建立 `.claude/code-specs/` 骨架，结构对齐 Trellis：`{package}/{layer}/` 二维布局 + 共享 `guides/`。
...
- 不负责填充具体 code-spec（那是 `/spec-update` 的职责）
...
4. 根据 framework 决定生成哪些 layer：
   ...
   - `guides/` 始终生成
5. 若检测到旧版 `.claude/code-specs/{frontend,backend}/` 顶层布局：
   - **不做自动迁移**
...
7. **Post-bootstrap Audit**（空模板审计）：
   ...
   - 输出一行计数：
     ...
     ```
   - **不**直接引导用户去跑 `/spec-update`——骨架刚生成就立刻沉淀过于激进。用户想填时自然会调用。
   - 与 `/scan` Part 5 的 bootstrap 提示语义不同：`/scan` 判断"是否需要 bootstrap"，本步判断"刚 bootstrap 出来的文件是否仍为空模板"。
   - 与 `/spec-review` 的 draft 检测也不冲突：review 盯长期存量，本步只看本次生成。
   - `CLAUDE_NON_INTERACTIVE=1`：仍打一行计数，不追问。

8. 告知下一步：
   - `/spec-update` 捕获第一条规范
   - `git add .claude/code-specs/ && git commit` 版本化骨架

## 用法

```
/spec-bootstrap              # 按 project-config 自动判断 packages × layers
/spec-bootstrap --force      # 即使没有检测到框架也生成 frontend + backend
/spec-bootstrap --reset      # 清空已有 .claude/code-specs/ 并重建（用于切换到新布局）
```
...
- `/spec-review` 检查骨架完整性、7 段合约完整性、canonical 模板升级差异
