...
若追溯失败 / 假设无法验证 → 标记 `manual_intervention` + `reason: root_cause_mismatch`（见 `references/manual-intervention-reasons.md`）。
...
单缺陷可能命中子集：`root_cause_mismatch` / `verification_failed` / `out_of_scope` / `review_rejected` / `user_rejected`。完整定义见 `references/manual-intervention-reasons.md`。
...
仅当入参有 `issue_number` 且验证通过时执行。按 `references/status-readiness.md § 流转动作` 通过 `bk transition_issue` 推进到 `处理中`，表明代码已落地。`manual_intervention` 路径跳过。失败处理按该文件"失败处理"矩阵。
...
按 `references/status-readiness.md § 判定条件` 评估（6 条全满足才 `true`）。
