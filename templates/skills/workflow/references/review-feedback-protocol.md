...
**讨论工件文本未转义**：Phase 0.2 的自由文本回答原样写入 `discussion-artifact.json` 并渲染到 spec.md。当前未做 Markdown 转义或 trust-boundary 隔离。在单人工作流场景中风险可接受（用户输入自己的数据）；若未来支持多人协作，需在 Spec 生成的讨论摘要渲染逻辑中对用户回答做引用块包裹或转义。
