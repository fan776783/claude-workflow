...
class AgentContract {
  /**
   * @param {object} options 合同配置
   * @param {string} options.name Agent 名称
   * @param {string} options.type Agent 类型
   * @param {string} options.version 版本号
   * @param {string} options.description 描述
   * @param {object[]} options.inputs 输入参数规范列表
   * @param {object[]} options.outputs 输出格式规范列表
   * @param {string[]} options.capabilities 能力声明列表
   * @param {string[]} options.constraints 约束条件列表
   ...
   * 验证值是否符合指定的输出格式
   * @param {*} value 待验证的值
   * @param {string} format 期望的输出格式（OutputFormat 枚举值）
  ...
  /**
   * 将合同的输入规范转换为 JSON Schema 格式
   * @returns {object} 符合 JSON Schema draft-07 的对象
   */
  toJsonSchema() {
