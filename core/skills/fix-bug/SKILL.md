...
灵感来自 mattpocock/skills `diagnose/SKILL.md` Phase 3——单假设容易 anchor 在第一个看似合理的想法上；多假设 + 可证伪预测 + 排序把"猜"变成"实验"。
...
**Step 2: 生成 3-5 个可证伪假设（ranked）**

- 基于追踪结果，生成 **3-5 个**候选假设并按概率排序
- 每个假设必须写明**可证伪预测**：
  - 格式：`If <X> 是因，then <改 Y> 会让 bug 消失` 或 `<改 Z> 会让 bug 加重`
  - **写不出预测 → 弃**（那是 vibe，不是假设）
- 输出为下面这张表：

| Rank | 假设 | 预测（If…then…） | 证伪成本 |
|------|------|------------------|----------|
| 1 | 因为 {原因}，导致 {组件} 在 {条件} 下 {错误} | If {X} 则 {Y} | 低 / 中 / 高 |
| 2 | … | … | … |
| … | … | … | … |

- `Rank 1` = 主假设；`Rank 2` = 备选假设；其余为候选（不强制验证，但若前两位被证伪可启用）
- 证伪成本用于 Step 4 选择先验证哪一条

**Step 3: 展示假设表并邀请校准**（非 Hard Stop）

- 把假设表打印给用户看
- **输出末尾显式加一行邀请**：`如需调整排序或补充假设，告诉我；否则我将按 rank 顺序开始证伪。`
- **不调用 AskUserQuestion**——Phase 2 才是唯一的 Hard Stop。这里只给用户自然的介入窗口
- 用户若在下一条消息里 push back（如 "Rank 1 不对，加一个 DB 超时假设"）→ 回到 Step 2 重排；不 push back → 直接进 Step 4
- **目的**：用户领域知识经常能瞬间重排优先级（cheap checkpoint, big time saver）

**Step 4: 最小化验证**

- 按 rank 顺序逐个证伪；证伪成本低的优先
- 设计最小实验验证，不直接进入修复
- 规则：
  - `Rank 1` 被证伪 → 验证 `Rank 2`
  - `Rank 1` 和 `Rank 2` 都被证伪 → 标记 `manual_intervention` + `reason: root_cause_mismatch`，不继续推进到 Phase 2
  - 前两位均未被证伪，但 `Rank 3+` 有更强证据 → 回 Step 2 重新排序（不算失败，不触发 `manual_intervention`）
  - 任一被证实 → 进入 Phase 2
...
- `无预测的"可能是 X"` — 是 vibe 不是假设（缺 If…then…）；拒绝进假设表
- `只列 1-2 个假设就开始证伪` — 违反 Step 2 的 3-5 要求，anchor 风险高
- `改了好几个地方应该能修好` — 散弹枪式修复
