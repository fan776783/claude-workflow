...
- 想看 ContextGovernor 如何决定继续/暂停/并行：看 Step 3 与 `../specs/execute/context-governor.md`
- 想看 task 完成后的后置管线：看 Step 6 与 `../specs/execute/post-execution-pipeline.md`
- 想看审查触发规则：看 Step 6 与 `../../workflow-reviewing/specs/execute/subagent-review.md`
- 想看统一 CLI / 脚本入口：看 `../../../specs/workflow-runtime/shared-utils.md`

## 何时读取

- 用户调用 `/workflow execute`
- 用户说“继续 / 下一阶段 / 重试 / 跳过”
...
**详细实现**: 参见 `../specs/execute/execution-modes.md`（路由索引），按需加载 `continuous-mode.md` / `phase-mode.md` / `retry-debugging.md` / `skip-mode.md`
...
创建后继续执行。参见 `../../../specs/workflow-runtime/state-machine.md` → 最小必需状态。
...
**详细实现**: 参见 `../../../specs/workflow-runtime/shared-utils.md` 与 `../../../utils/workflow/path_utils.js`
...
每个 task 执行完成后，必须依次完成以下 6 步管线。**详细步骤和 checklist 参见 `execution-checklist.md`（唯一权威源）。**
...
| ⑥    | Journal 记录             | 在质量关卡/暂停/完成时调用 `workflow_cli.js journal add` 记录会话进展              |
...
使用 `../../../utils/workflow/workflow_cli.js journal add` 持久化会话进展，确保新 Session 启动时可恢复上下文：

```bash
node ../../../utils/workflow/workflow_cli.js journal add \
  --title "完成 T1-T3 任务" \
...
**恢复使用**：新 Session 启动时可通过 `node core/utils/workflow/workflow_cli.js journal list` 查看最近进展，或 `node core/utils/workflow/workflow_cli.js journal search "关键词"` 检索特定上下文。
...
**详细实现**: 参见 `../specs/execute/retry-debugging.md`
...
**详细实现**: 参见 `../specs/execute/skip-mode.md`
...
**详细实现**: 参见 `../../workflow-reviewing/specs/execute/actions/quality-review.md`
...
**详细实现**: 参见 `../../../specs/workflow-runtime/external-deps.md`、`../../../specs/workflow-runtime/state-machine.md` 与 `../../../utils/workflow/dependency_checker.js`
...
- `execution-modes.md` - 执行模式索引（链接到各子文件：continuous-mode / phase-mode / retry-debugging / skip-mode / context-governor / subagent-routing / post-execution-pipeline / tdd-enforcement）
