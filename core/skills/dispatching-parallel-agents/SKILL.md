...
将任务按**上下文边界**而非抽象角色拆分，再按平台能力路由到合适的subagent。
...
- 需要把独立调查 / 修复 / 分析任务并行分派给多个subagent，并由主会话统一汇总
- 需要显式限制每个subagent 只处理单一问题域，避免共享状态和上下文污染

如果只是**单个任务隔离执行**、**单个 reviewer subagent review**、或任务间仍有共享文件 / 共享状态 / 显式依赖，则不要使用本 skill，直接走普通单subagent 或顺序执行。
...
2. **每个subagent 只接收最小必要上下文**，不得继承整段主会话历史。
...
5. 若只有 1 个任务，但任务上下文很重，也可以使用单subagent 隔离执行
...
4. **导入独立**：同批次一个任务创建的module若会被另一任务立即导入，则不并行
...
在真正启动并行subagent 之前，先完成本批次的隔离准备。这里的目标不是开始执行任务，而是**只处理会触发共享 Git 元数据写入的 provisioning**。
...
- provisioning 完成后，再进入并行subagent 启动阶段
...
- provisioning 完成后，再使用后台运行启动并行subagent
- 主会话逐个等待结果并记录成功/失败
- Codex 类平台在回收后显式执行清理
...
2. 若出现冲突或验证失败：
   ...
   - 保留subagent 产物与诊断信息，但不保留“该批次已完成”的最终结论
   - 按原顺序重新顺序执行
3. 若仅个别任务失败：
   - 失败任务标记 `failed`
   - 其余成功任务保留结果
...
3. `execute` 的 Step 6：完成 provisioning 后，再把已确认独立的并行批次路由到多subagent
...
- 独立性不明确 → 顺序执行
- 并行后冲突 → 回退顺序执行
- 任意subagent 输出缺少验证证据 → 不得标记对应任务完成
...
而非在 execute skill 内部直接创建 worktree 或启动subagent。`batch.dispatch_skill` 字段标识了应使用的 skill 名称。
