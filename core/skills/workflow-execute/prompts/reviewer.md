...
verification: { command, result, output_summary }   # implementer 跑的验证证据（取自报告文件）
（status/summary/files_changed 仅作定位线索；一切判断以 diff 为准，不要把 implementer 的 summary/status 当作事实）
（**验证证据裁决**：controller 不再自跑测试，你须确认 verification 的 command 与改动相关、result=pass、output 干净——**output 有 warning/noise 即 finding**；对实现有具体疑点时跑 **focused 单测**确认，绝不跑 package 级全套 / race / 高频循环）
</implementer-output>
...
clean PASS 形态：
{
  ...
  "verification_review": { "command_relevant": true, "result_pass": true, "output_clean": true }
}
...
REVISE / gap 形态：
{
  ...
  "verification_review": { "command_relevant": true, "result_pass": false, "output_clean": true, "focused_recheck": "<跑过的 focused 单测/命令，没跑则 null>" },
...
>
> **verification_review 职责（取代 controller 自跑验证，HARD-GATE #2 的机械门）**：controller 不再自跑测试,由你对 implementer 报告的 `verification`（command/result/output）裁决,**必填**三布尔:
> - `command_relevant`：报告的验证命令是否真覆盖本 task 改动（非无关命令 / 空跑 / 占位）
> - `result_pass`：result 是否 pass
> - `output_clean`：output 是否干净（warning/noise = false，并按 finding 记入 phase2）
>
> 三者**全 true** 才允许 PASS;任一为 false → `decision` 必须 REVISE（implementer 报告的验证不可信,回修）。有具体疑点时跑 **focused 单测**确认并填 `focused_recheck`（没跑填 `null`）。**省略 `verification_review` = schema 违规**(controller 重派 1 次)——这是防"伪造 result:pass 蒙混过 advance gate"的唯一机械门,替代了原 controller 自跑验证。
...
- **枚举 / file:line 形态校验**（O2a）：
  ...
  - **verification_review 必填**（F2，HARD-GATE #2 机械门；**per-task 限定**，final-review 不适用见末尾形态）：`verification_review` 缺失 → schema violation 重派 1 次。三布尔任一为 false 而 `decision: PASS` → 自相矛盾,视同 schema violation 重派。controller **仅在三者全 true 时**才 `verification.js create --passed` + advance;任一 false → 按 REVISE 回 implementer（**不 advance**）——取代了 controller 自跑验证的 anti-fabrication 职能。
- **REVISE 后（fresh re-dispatch，O4）**：把 `revise_instructions` 塞回 implementer prompt → **重派 fresh implementer + fresh reviewer subagent**（复用同一 reviewer 模板，不分 spec/质量两轮）。**禁** `SendMessage` / transcript-resume 复用既有 subagent（O4，理由见 [`../references/subagent-driven.md`](../references/subagent-driven.md)「不允许的行为」）。每轮 REVISE 都是干净新派发，只喂 `revise_instructions` + 该 task scoped diff。
...
- **[MODEL] 分级提示**：reviewer **至少中档**（sonnet-class）——廉价模型无法构造有效 failure_scenario，refute-default 形同虚设；final-review 整 branch 终审用最强（opus-class）。tier 细则见 [`../SKILL.md`](../SKILL.md) § 模型分级。平台不支持指定模型时忽略。
...
> per-task review 结果**不落盘为 durable gate 记录**（`quality_review pass`/`fail` 持久化已退役）；PASS + 验证通过后的 review 结论（clean/issues + known-issues）经 progress ledger 持久化作 advisory recovery（Step 5 ④ 写、/clear 后 Step 2 `progress-ledger read` 读回，**非 gate 记录**），REVISE 中间轮次仅活在 controller 本会话内存。
...
| `files_changed` | controller | implementer 报告文件 `implementer-report.json` 的 `files_changed`（**权威源**）。**thin 回执不含此字段**——回执仅作路由信号；controller 装配 reviewer 前**读报告文件取 `files_changed`**（normal path 也读，非仅恢复）。final-review 改为已完成 task 清单 + 执行阶段决策蒸馏 Decisions/Rejected/Risks + 已知问题排除清单（per-task review minors + concerns；由 progress ledger 持久化，/clear 后 Step 2 `progress-ledger read` 读回），controller 从本会话内存 + progress ledger 装配 | 注入 `<implementer-output>` 段 |
