...
<CONTEXT>
写用户可见文档正文 / Base 数据时 Read `core/specs/shared/glossary.md`。纯 CLI 转发（auth/list-tools/只读调用）可跳过。
</CONTEXT>

# alidocs — 钉钉文档 + AI 表格 + 钉钉表格 CLI
...
| 用户模糊说"操作钉钉文档"，既没 URL 也没提 dws | **先问**："你想用 MCP URL 直连，还是用 dws CLI？如果你有对应 MCP server URL，我可以走 alidocs skill" |
...
调用形式：
```bash
node <skill-root>/alidocs/cli/dingtalk-mcp.mjs <subcommand> [args...]
```
...
参考文档（`references/{doc,aitable,sheet}.md`）是**便于理解用途**，不是权威 contract。真参数以下面命令的输出为准：
...
**AskUserQuestion enum 来自 cache 时强制 refresh**（ADR-0001 Decision 8）：让用户从 enum 选择前（如 singleSelect 字段值 / state 名），若候选 enum 取自 schema cache，先跑一次 `list-tools --refresh` 或对应 introspection（`get_field` / `list_fields`），避免 server 漂移后让用户选中已废值。
...
4. 疑似破坏性工具未被硬门拦截 → troubleshooting.md 里有上报指引，按 workflow 提 issue

## 工具清单（仅备查，实时数量以 `list-tools` 为准）

<!-- snapshot 2026-05-15 — counts: doc=26, aitable=47, sheet=37 (server may add tools; baseline diff via `diff-tools` flags drift). refresh: `list-tools --refresh`. See ADR-0001 Decision 1/2. -->

- 文档 MCP（doc）：~26，按 "创建 / 读取 / 编辑 / 组织 / 附件" 分类 → `references/doc.md`
- AI 表格 MCP（aitable）：~47，按 "Base / Table / Field / Record / View / Chart / Dashboard / Import-Export / Attachment" 分组 → `references/aitable.md`
- 钉钉表格 MCP（sheet）：~37，按 "工作簿 / 工作表 / 区域 / 查找替换 / 筛选 / 筛选视图 / 行列 / 合并图片" 分组 → `references/sheet.md`
- 实时 schema：`list-tools --refresh` / `schema <kind>.<tool>`
- 漂移检测：`diff-tools` 对比 `core/skills/alidocs/baseline-schema.json`（checkin baseline，三 kind 合一）
...
- [references/troubleshooting.md](./references/troubleshooting.md) — 错误码 + 常见坑 + 破坏性工具上报 workflow
