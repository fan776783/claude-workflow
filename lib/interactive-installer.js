...
async function runInteractiveStatus(options = {}) {
  ...
  for (const [name, agentStatus] of Object.entries(status.agents)) {
    ...
    if (name === 'claude-code' && agentStatus.installed) {
      const projectLevel = !global;
      const worktreeStatus = describeHookStatus(agentStatus.worktreeHooks, { projectLevel });
      const workflowBaseStatus = describeHookStatus(agentStatus.workflowHooks?.base, { projectLevel });
      const workflowStrictStatus = describeHookStatus(agentStatus.workflowHooks?.strict, { optional: true, projectLevel });
      lines.push(`      worktree hooks: ${worktreeStatus}`);
      lines.push(`      workflow hooks(base): ${workflowBaseStatus}`);
      lines.push(`      workflow hooks(strict): ${workflowStrictStatus}`);
    }
  }
