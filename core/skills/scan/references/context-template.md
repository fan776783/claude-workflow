...
>
> **精简原则**：下方模板是**字段清单**而非排版规范——按它确定写哪些事实，但格式要压到最紧。优先**内联列表**（`a · b · c` / `key(说明)`）而非每项一行的多列表格；表格仅在**多行查表**（每行一个独立条目、需逐项对照，如路由清单）或 3+ 列且每列都承载信息时保留，2-3 项的小表一律内联。**不写占位行**（无内容的维度直接删整节，例如纯前端项目删「数据模型」「测试覆盖」）。能 `X → Y` 表达流程就不分点。目标是高密度参考卡，不是逐项文档；同一事实只占一行。
