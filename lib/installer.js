...
/**
 * 为 Agent 派发 hooks.json：读 core/hooks/agent-templates/<template>，把 {{HOOKS_DIR}}
 * 替换为该 agent managedDir 下 hooks 链接的绝对路径，写到 agent 期望位置。
 * 仅对声明了 hooksTemplate 的 agent 生效（cursor / codex / github-copilot）。
 * @param {string} sourceRoot - core 根目录
 * @param {string} agentName - Agent 名称
 * @param {string} agentManagedDir - Agent 受管命名空间目录（hooks 链接在其下）
 * @param {string} hooksFilePath - 目标 hooks.json 绝对路径
 * @returns {Promise<{success: boolean, path: string, template?: string, error?: string}>}
 */
async function installAgentHooksConfig(sourceRoot, agentName, agentManagedDir, hooksFilePath) {
  ...
  try {
    ...
    const rendered = raw.replace(/\{\{HOOKS_DIR\}\}/g, hooksDirAbs);
    await fs.ensureDir(path.dirname(hooksFilePath));
    await fs.writeFile(hooksFilePath, rendered, 'utf8');
    return { success: true, path: hooksFilePath, template: agent.hooksTemplate };
  } catch (err) {
...
async function linkToAgents(sourceRoot, agentNames, options = {}) {
  ...
  for (const agentName of agentNames) {
    ...
    if (hooksFilePath) {
      agentResult.hooksConfig = await installAgentHooksConfig(
        sourceRoot, agentName, agentManagedDir, hooksFilePath,
      );
      if (agentResult.hooksConfig && agentResult.hooksConfig.success === false) {
