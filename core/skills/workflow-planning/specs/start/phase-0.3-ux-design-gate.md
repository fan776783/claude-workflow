...
- 想看何时触发 UX gate：看“触发条件”
- 想看流程图/页面分层要求：看后续设计产物章节
- 想看用户审批循环：看 review / revise 相关章节
- 想看 artifact 持久化：搜 ux-design-artifact

## 何时读取

- 需求涉及前端/UI/交互，可能触发 HARD-GATE 时
- 需要确认设计审批必须先于 Spec 生成时
...
- **一旦 Phase 0.3 被触发，必须完成用户审批后才能进入 Phase 1 Spec 生成**。不存在静默跳过路径。
- 若用户明确要求取消 UX 设计，应回退到 Phase 0.2 重新讨论需求范围（确认是否真的不需要 UI），而非在本阶段引入跳过标记。
