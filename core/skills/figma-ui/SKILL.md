...
构建 `taskDir/change-manifest.md`（详细执行见 [`references/change-playbook.md`](references/change-playbook.md)）：
...
- **C.2a 残值清零** — 用 B.0 定死的 residue pattern 对账 preCount → afterCount，不归零且无 justify = P0 → `_residue.md`
- **C.2b Diff 双射** — 每个 git diff hunk 映射到 entry / 传播决策 / AssetPlan，未映射 = scope creep P0 → `_coverage.md`
- **Visual Review / 修复循环 / 交付决策** — 同 CREATE 分支 step 6-8；交付摘要必须引用 `_coverage.md` / `_residue.md` 实际内容
...
| "residue 没归零但应该是巧合" | 逐条 justify 进 `_residue.md`，禁止批量豁免 |
