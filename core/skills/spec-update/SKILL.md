---
name: spec-update
description: "显式捕获学到的内容并写入 .claude/code-specs/（v2.2 对齐 Trellis）。按主题写入 convention 或 contract 文件，分基础/深度更新两条路径，对齐 Trellis 03-11 PRD。"
---

# /spec-update

显式、用户驱动的 code-specs 沉淀入口。对齐 Trellis `update-spec` 的交互式模式。

## v2.2 核心变化

- **模板分两档**：convention（轻量必备 4 段 + 可选扩展）为主力，contract（7 段）仅用于 API/DB/字段级契约
- **基础更新 vs 深度更新**：对齐 Trellis 归档 PRD `03-11-improve-break-loop-update-spec`，简单追加不强制 7 段 checklist
- **主题 fuzzy 匹配**：写入前查找 `{pkg}/{layer}/*.md` 相似主题，提示追加还是新建
- **6 类标签不进段落**：作为交互期语义标签，只用于"建议用哪个模板"+"正文首行 `> Type: X` 注释"
- **index.md 严格三列**：`| Guide | Description | Status |`（对齐 Trellis live），不加 Type 列、不做 From→To 日志
...
### Step 1: 基础 vs 深度更新分流（对齐 Trellis 03-11 PRD）
...
- 更新 `{pkg}/{layer}/index.md` 的 Guidelines Index 表：
  ...
  - 表头严格三列（Guide / Description / Status），对齐 Trellis live
