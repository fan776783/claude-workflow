...
- `templates/skills/diff-review/SKILL.md` 进入 finding verification 之后、报告汇总之前
...
本规范复用了以下已有模式：
- `templates/skills/fix-bug/references/impact-analysis.md` 的依赖链 / 数据流 / 测试覆盖 / 风险分级
- `templates/skills/workflow-delta/specs/delta/impact-analysis.md` 的 `affectedFiles / affectedModules / riskLevel` 结构化思路
- `templates/specs/workflow-runtime/review-feedback-protocol.md` 的 `VERIFY → EVALUATE` 纪律
