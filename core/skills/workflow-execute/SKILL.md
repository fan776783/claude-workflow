...
3. **按选择分流**:
   ...
   - `skip` → `execution_sequencer.js skip <state-path> <plan-path> {taskId}`（`<plan-path>` 为退役占位参数,状态落 task-dir + state.json）,reason 写入 journalSummary
...
- `<protocols>`:TDD 开关 / 输出协议;HITL task 追加 `"Before any code change, you MUST emit NEEDS_CONTEXT with the questions the human needs to answer."`(HARD-GATE #5)
...
CLI 自动标记 `skipped` + 更新 task-dir(task.json) + state.json + 找下一任务（`<plan-path>` 为退役占位参数,不再改写 plan.md;仅 legacy plan.md workflow 回写 plan.md）。
