...
async function linkToAgents(sourceRoot, agentNames, options = {}) {
  ...
  for (const agentName of agentNames) {
    ...
    // 4 个 per-agent 操作目标 disjoint 目录（commands/agent-workflow / .agent-workflow/{hooks,specs,utils} / skills / hooks.json）。
    // 并行启动节省 ~50ms/agent。
    const hooksFilePath = getAgentHooksFile(agentName, global, cwd);
    const [commandsResult, managedDirsResult, skillsResult, hooksConfigResult] = await Promise.all([
