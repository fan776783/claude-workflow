...
- `requirement-baseline.json`（需求保真层；提供 must_preserve 细节、acceptance_signal 和 requirement coverage 输入）
...
- **Requirement Coverage First** — 先确认 requirement baseline / spec traceability，再拆解文件和步骤
...
- **Protected Nuances Stay Visible** — baseline 中 `must_preserve` 的细节必须在 Requirement Coverage、任务步骤或验证命令中找到落点
...
const requirementBaseline = readRequirementBaseline(workflowDir);
const requirementCoverage = deriveRequirementCoverage(specContent, requirementBaseline, slices);
```
...
for (const slice of slices) {
  for (const file of slice.files) {
    tasks.push({
      ...
      requirement_ids: slice.requirementIds || deriveRequirementIdsForSlice(slice, requirementCoverage),
      critical_constraints: deriveProtectedNuancesForSlice(slice, requirementBaseline),
...
const planContent = replaceVars(planTemplate, {
  ...
  requirement_coverage: renderRequirementCoverage(requirementCoverage, tasks),
...
### Step 4.6: Requirement Coverage Drift Check

```typescript
// requirement baseline 中的 must_preserve 细节不得在 plan 阶段失踪
for (const requirement of requirementBaseline.filter((req) => req.scope_status === 'in_scope')) {
  const covered = tasks.some((task) => (task.requirement_ids || []).includes(requirement.id));
  if (!covered) {
    console.error(`❌ Coverage Drift: in-scope requirement "${requirement.id}" 未映射到任何 Plan Task`);
    console.log('⏸️ 请回退到 Phase 1 / Phase 2，补充 Requirement Traceability 或 Plan Task 覆盖后重新生成 Plan');
    return;
  }
}

for (const protectedRequirement of requirementBaseline.filter((req) => req.must_preserve)) {
  const visible = tasks.some((task) =>
    (task.requirement_ids || []).includes(protectedRequirement.id)
      || (task.critical_constraints || []).some((constraint) => constraint.includes(protectedRequirement.id))
  );
  if (!visible) {
    console.error(`❌ Coverage Drift: must_preserve 细节 "${protectedRequirement.id}" 未在 Plan 中保留`);
    console.log('⏸️ 请补充 Requirement Coverage、关键约束或验证步骤后重新生成 Plan');
    return;
  }
}
```
...
function selfReviewPlan(planContent: string, specContent: string): void {
  // 1. Requirement coverage: 逐条检查 requirement baseline / spec 的每个需求
  ...
  for (const req of specRequirements) {
    const covered = planTasks.some(
      (task) =>
        (task.requirement_ids || []).includes(req.id) || task.spec_ref === req.sectionRef,
    );
    if (!covered) {
  ...
  // 1.5 Protected nuance coverage: must_preserve 细节必须在 task / verification 中可见
  const baselineRequirements = readRequirementBaseline(workflowDir);
  for (const req of baselineRequirements.filter((item) => item.must_preserve)) {
    const preserved = planTasks.some(
      (task) =>
        (task.requirement_ids || []).includes(req.id)
          || (task.critical_constraints || []).some((constraint) => constraint.includes(req.id)),
    );
    if (!preserved) {
      console.warn(`⚠️ must_preserve 细节 [${req.id}] 未在 plan 中保留`);
    }
  }
