...
状态字段扩展（详见 `core/specs/workflow-runtime/state-machine.md`）：
...
### 5.1 `/diff-review` 的三种模式

`/diff-review` 通过参数在三种范围来源之间切换：

| 模式 | 触发 | 变更集来源 | 上游合并 / 脏文件 | Compaction / `/clear` |
|------|------|-----------|-------------------|----------------------|
| 默认（staged） | `/diff-review` | `git diff --staged` | 可能一起进入 | 不敏感 |
| branch | `/diff-review --branch <base>` | `git diff <base>...HEAD` | 可能一起进入 | 不敏感 |
| session | `/diff-review --session` | 当前会话里本模型的 Edit / Write / NotebookEdit 记录 | 显式排除，只审本轮改动 | 硬停：检测到压缩或清空立即中止，不回退到 git |

> 旧版 `/session-review` 已合并入 `--session` 模式，原 skill 已下线。

共享管线：Layer C-H（Candidate Discovery → Normalization → Verification → Impact Analysis → Severity Calibration → Report Synthesis）统一走 `core/skills/diff-review/specs/review-pipeline.md`，三种模式在 prompt 里都会对 Codex 显式限定审查范围。
...
仓库当前提供 24 个 skill 目录，按职责分为四类（`/team` 已下沉为 Claude Code 原生命令，不再作为 skill）：
...
| `diff-review` | `/diff-review [--branch <base> \| --session]` | Impact-aware 代码审查（staged / branch / session 三种模式；含 finding verification、影响性分析、fix/skip 复审循环） |
| `bug-batch` | `/bug-batch` | 批量缺陷分析与分组修复 |
| `figma-ui` | `/figma-ui` | Figma 设计稿到代码 |
| `research` | `/research` | 统一研究入口（代码库 / 生态 / 外部引文，合并自旧 `search-first` + `deep-research`） |
| `grill` | `/grill` | 需求对齐澄清，替代旧 `/enhance` |
| `quick-plan` | `/quick-plan` | 轻量快速规划（从旧 command 迁 skill） |
| `diagnose` | `/diagnose` | 疑难 Bug 反馈循环 + 假设证伪（不改代码） |
| `tdd` | `/tdd` | Red-green-refactor vertical slice TDD 纪律 |
| `zoom-out` | `/zoom-out` | 抬升抽象层，画 module / caller 地图 |
| `write-a-skill` | `/write-a-skill` | Meta-skill：新建 / 审查 SKILL.md |
...
#### 5.3.3 规划与研究辅助 Skills

轻量规划 / 对齐 / 研究 / TDD 纪律等辅助能力已在 5.3.1 的专项 Skills 表里列齐（`quick-plan`、`research`、`grill`、`zoom-out`、`diagnose`、`tdd`、`write-a-skill`）。这里单列一个仅由 workflow / team 内部触发的辅助 skill：
...
- **Commands**（`core/commands/`）：`team`、`git-rollback`（其余已下沉为 skill）
...
# 轻量规划与辅助命令
/quick-plan "需求描述"
/grill "需求描述"                                     # 对齐澄清（替代旧 /enhance）
/git-rollback

# 专项 skill
/fix-bug "bug 描述"
/diagnose                                            # 疑难 Bug 根因证伪（不改代码）
/tdd                                                 # Red-green-refactor TDD 纪律
/diff-review                                         # 默认：git diff --staged
/diff-review --branch main                           # 分支差异
/diff-review --session                               # 仅审本会话内由本模型改过的文件（合并自旧 /session-review）
/bug-batch
/figma-ui <URL>
/research "功能需求 / 研究主题"                        # 合并自旧 /search-first + /deep-research
/zoom-out                                            # 抬升抽象层，画地图
/write-a-skill                                       # 新建 / 审查 SKILL.md
...
- `core/skills/research/SKILL.md`（合并自旧 search-first + deep-research）
- `core/skills/grill/SKILL.md`、`core/skills/zoom-out/SKILL.md`、`core/skills/diagnose/SKILL.md`、`core/skills/tdd/SKILL.md`、`core/skills/write-a-skill/SKILL.md`
- `core/skills/diff-review/SKILL.md`（含 `--session` 模式，合并自旧 session-review）
...
- `core/specs/workflow-runtime/state-machine.md`（含 ParallelExecution / ParallelGroupRecord / BatchQualityGateResult）
