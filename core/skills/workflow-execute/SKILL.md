...
**task 源 = task-dir**(`~/.claude/workflows/{pid}/tasks/{taskId}/{task.json,context.jsonl}`),非 plan.md 物理解析。确认 state(下方 Step 2)后,controller **一次性**从 task 源(`TaskSource(state).listTasks()`)读全部 task,在本会话内存里持有**所有** task 切片(每个 task 的元数据全文 + acceptance + constraints + patterns/mandatory-reading + files)。**不每 task 重读**,也不再每 task 调 `task-bundle`(对齐 superpowers controller-持全 范式,见 [`references/subagent-driven.md`](references/subagent-driven.md))。
