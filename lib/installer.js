...
// Agent memory 文件：随 canonical 落盘（installToCanonical → <canonical>/core/），
// 再由 bin sync 按各工具原生文件名分发到对应 config home。
// CLAUDE.md→Claude（claude-code-plugin 负责写 ~/.claude/），
// AGENTS.md→Codex/Cursor/Droid/Copilot/OpenCode，GEMINI.md→Antigravity（见 lib/agents.js globalMemory）。
const TEMPLATE_FILES = ['CLAUDE.md', 'AGENTS.md', 'GEMINI.md'];
...
// 受管 subagent manifest：仅声明了 agentsDir 的工具（如 Qoder ~/.qoder/agents/）才落盘，
// 与 claude-code-plugin.js 清理用的历史文件名一致。
...
/**
 * 获取受管 subagents manifest 文件路径
 * @param {string} agentAgentsDir - Agent subagents 目录
 * @returns {string} manifest 文件路径
 */
function getManagedAgentsManifestPath(agentAgentsDir) {
  return path.join(agentAgentsDir, MANAGED_AGENTS_MANIFEST);
}

/**
 * 读取受管 subagents manifest
 * @param {string} agentAgentsDir - Agent subagents 目录
 * @returns {Promise<Object>} manifest 中的 entries 对象
 */
async function readManagedAgentsManifest(agentAgentsDir) {
  const manifestPath = getManagedAgentsManifestPath(agentAgentsDir);
  if (!(await fs.pathExists(manifestPath))) {
    return {};
  }
...
/**
 * 写入受管 subagents manifest
 * @param {string} agentAgentsDir - Agent subagents 目录
 * @param {Object} entries - 安装结果条目
 */
async function writeManagedAgentsManifest(agentAgentsDir, entries) {
  const manifestPath = getManagedAgentsManifestPath(agentAgentsDir);
  const managedEntries = Object.fromEntries(
    Object.entries(entries)
      .filter(([, result]) => result?.success)
...
async function getSkillEntries(sourceRoot) {
  ...
  return entries
    ...
    // mounted via relative path between sibling skills, not standalone skills.
    // Skip them from per-skill mount.
...
/**
 * 获取 subagent 条目（core/agents/*.md 顶层 .md 文件）
 * 与 getCommandEntries 同形：返回 { name, target } 列表。
 * @param {string} sourceRoot - 源根目录
 * @returns {Promise<Array<{name: string, target: string}>>}
 */
async function getAgentEntries(sourceRoot) {
  const agentsRoot = path.join(sourceRoot, AGENTS_DIR);
  ...
  return entries
    ...
    .map(entry => ({
      name: entry.name,
      target: path.join(agentsRoot, entry.name),
    }))
    .sort((a, b) => a.name.localeCompare(b.name));
}
...
/**
 * 为 Agent 链接 subagent 条目（清理过期条目 + 创建新链接 + 更新 manifest）
 * 仅声明了 agentsDir 的工具会调用（如 Qoder ~/.qoder/agents/）。与 linkCommandEntries 同形：
 * 顶层 .md 逐个 symlink（Windows 下文件 junction 失败会回退复制），独立 manifest 管理 stale 清理。
 * @param {string} sourceRoot - 源根目录
 * @param {string} agentAgentsDir - Agent subagents 目录
 * @param {boolean} fallbackToCopy - 链接失败时是否回退到复制
 * @returns {Promise<{rootMode: string, entries: Object, removed: string[], count: number}>}
 */
async function linkAgentEntries(sourceRoot, agentAgentsDir, fallbackToCopy = true) {
  const agentEntries = await getAgentEntries(sourceRoot);
  const agentEntryNames = new Set(agentEntries.map(entry => entry.name));

  await fs.ensureDir(agentAgentsDir);

  const removed = [];
  const manifestEntries = await readManagedAgentsManifest(agentAgentsDir);
  const existingEntries = await fs.readdir(agentAgentsDir, { withFileTypes: true });

  for (const entry of existingEntries) {
    if (entry.name === path.basename(getManagedAgentsManifestPath(agentAgentsDir))) {
      continue;
    }

    if (!agentEntryNames.has(entry.name)) {
      const entryPath = path.join(agentAgentsDir, entry.name);
      if (manifestEntries[entry.name]) {
        await fs.remove(entryPath);
        removed.push(entry.name);
      }
  ...
  for (const entry of agentEntries) {
    const linkPath = path.join(agentAgentsDir, entry.name);
    links[entry.name] = await createSymlink(entry.target, linkPath, fallbackToCopy);
  }

  await writeManagedAgentsManifest(agentAgentsDir, links);
...
const { buildBackupTimestamp } = require('./claude-code-plugin');

// Shell-safe quote: 双引号包 + 转义内部 "。POSIX 和 Windows cmd/powershell 都接受双引号。
function shellQuotePath(p) {
  if (!/[^A-Za-z0-9_\-./:\\]/.test(p)) return p;
  return '"' + p.replace(/"/g, '\\"') + '"';
}

function renderCommandTemplate(commandTemplate, hooksDirAbs) {
  return commandTemplate.replace(/\{\{HOOKS_DIR\}\}\/([A-Za-z0-9_\-]+\.js)/g, (_, script) => {
    return shellQuotePath(path.join(hooksDirAbs, script));
  });
}

// 递归遍历，对 command / bash / powershell 字段做 {{HOOKS_DIR}} 替换。
function substituteCommandPaths(node, hooksDirAbs) {
  if (Array.isArray(node)) {
    for (const item of node) substituteCommandPaths(item, hooksDirAbs);
    return;
  }
  if (!node || typeof node !== 'object') return;
  for (const [key, value] of Object.entries(node)) {
    if (typeof value === 'string' && (key === 'command' || key === 'bash' || key === 'powershell')) {
      node[key] = renderCommandTemplate(value, hooksDirAbs);
    } else {
      substituteCommandPaths(value, hooksDirAbs);
    }
  }
}

// 提取一个 hook entry 的 command 串（同时检 entry.command / entry.bash / entry.powershell / entry.hooks[].command）
function collectEntryCommands(entry) {
  const cmds = [];
  if (!entry || typeof entry !== 'object') return cmds;
  for (const k of ['command', 'bash', 'powershell']) {
    if (typeof entry[k] === 'string') cmds.push(entry[k]);
  }
  if (Array.isArray(entry.hooks)) {
    for (const inner of entry.hooks) {
      for (const k of ['command', 'bash', 'powershell']) {
        if (inner && typeof inner[k] === 'string') cmds.push(inner[k]);
      }
    }
  }
  return cmds;
...
// 过滤同 event 下属于我们的 entry（命令引用了我们的脚本名）。保留用户在同 event 添加的其它 entry。
// 仿 lib/claude-code-plugin.js filterManagedHooksInEvent 的 per-entry 过滤策略。
function filterManagedEntries(entries, managedScriptNames) {
  if (!Array.isArray(entries)) return [];
  return entries.filter((entry) => {
    const cmds = collectEntryCommands(entry);
    if (cmds.length === 0) return true;
    return !cmds.some((cmd) => managedScriptNames.some((name) => cmd.includes(name)));
  });
}

// 从已渲染的 template 中提取受管脚本文件名集合（用于在 sync 时识别哪些既存 entry 是我们的）
function extractManagedScriptNames(node, acc = new Set()) {
  if (Array.isArray(node)) {
    for (const item of node) extractManagedScriptNames(item, acc);
    return acc;
  }
  if (!node || typeof node !== 'object') return acc;
  for (const [key, value] of Object.entries(node)) {
    if (typeof value === 'string' && (key === 'command' || key === 'bash' || key === 'powershell')) {
      const m = value.match(/([A-Za-z0-9_\-]+\.js)/g);
      if (m) for (const name of m) acc.add(name);
    } else {
      extractManagedScriptNames(value, acc);
    }
  }
  return acc;
}
...
/**
 * 派发 agent hooks.json：渲染模板 → 与既存配置 merge → 写盘 + 更新 sidecar manifest。
 *
 * 关键设计：
 *   - 用 JSON 对象操作（不是字符串模板拼接），避免 Windows backslash 在 JSON string 里成非法 escape，
 *     也避免 POSIX 含空格路径被 shell tokenize 错。
 *   - sidecar `<hooksFile>.agent-workflow.json` 记上次受管 event + 受管脚本名，下次 sync 据此移除旧我方 entry。
 *   - 同一 event 下的 entry 用脚本名做 per-entry 过滤（仿 claude-code-plugin filterManagedHooksInEvent），
 *     保留用户在 sessionStart 等 event 上自加的非我方 entry。
 *   - 首次安装若既存 hooks.json 与我方 event 冲突且无 sidecar 标识 → 备份后再写。
 */
async function installAgentHooksConfig(sourceRoot, agentName, agentManagedDir, hooksFilePath) {
  const agent = agents[agentName];
  if (!agent || !agent.hooksTemplate || !hooksFilePath) {
    return { success: true, skipped: true };
  }
  const templatePath = path.join(sourceRoot, 'hooks', 'agent-templates', agent.hooksTemplate);
  try {
    let templateObj;
    try {
      templateObj = await fs.readJson(templatePath);
    } catch (err) {
      if (err.code === 'ENOENT') {
        return { success: false, path: hooksFilePath, error: `template missing: ${templatePath}` };
      }
      throw err;
    }
    const hooksDirAbs = path.join(agentManagedDir, 'hooks');
    substituteCommandPaths(templateObj, hooksDirAbs);
    const managedEvents = Object.keys(templateObj.hooks || {});
    const managedScriptNames = [...extractManagedScriptNames(templateObj.hooks || {})];

    const sidecarPath = `${hooksFilePath}.agent-workflow.json`;
    const sidecar = await readJsonSafe(sidecarPath);
    const prevManagedScripts = Array.isArray(sidecar?.managedScripts) ? sidecar.managedScripts : managedScriptNames;
    // 上次安装时声明过的脚本 + 本次将装的合并：避免 rename 模板时旧 entry 残留
    const dropScripts = [...new Set([...prevManagedScripts, ...managedScriptNames])];

    const existingTopLevel = await readJsonSafe(hooksFilePath).catch(() => null);
    let mergedHooks = {};
    let backedUp = null;
    let parseFailed = false;

    if (existingTopLevel === null && await fs.pathExists(hooksFilePath)) {
      // 文件存在但 readJsonSafe 拒绝（JSON 损坏）→ 备份重写
      parseFailed = true;
      backedUp = `${hooksFilePath}.bak.${buildBackupTimestamp()}`;
      try { await fs.copy(hooksFilePath, backedUp); } catch { backedUp = null; }
    } else if (existingTopLevel && existingTopLevel.hooks && typeof existingTopLevel.hooks === 'object') {
      mergedHooks = { ...existingTopLevel.hooks };
      for (const ev of managedEvents) {
        if (Array.isArray(mergedHooks[ev])) {
          mergedHooks[ev] = filterManagedEntries(mergedHooks[ev], dropScripts);
          if (mergedHooks[ev].length === 0) delete mergedHooks[ev];
        }
      }
      // 首次安装（无 sidecar）且过滤后仍有 entry 引用 → 备份保护用户原始文件
      if (!sidecar) {
        const conflicts = managedEvents.filter((e) => Array.isArray(existingTopLevel.hooks[e]) && existingTopLevel.hooks[e].length > 0);
        if (conflicts.length > 0) {
          backedUp = `${hooksFilePath}.bak.${buildBackupTimestamp()}`;
          await fs.copy(hooksFilePath, backedUp);
        }
      }
    }

    for (const event of managedEvents) {
      mergedHooks[event] = [...(mergedHooks[event] || []), ...templateObj.hooks[event]];
    }

    const output = !parseFailed && existingTopLevel && typeof existingTopLevel === 'object'
      ? { ...existingTopLevel, ...templateObj, hooks: mergedHooks }
      : { ...templateObj, hooks: mergedHooks };

    await fs.ensureDir(path.dirname(hooksFilePath));
    await fs.writeJson(hooksFilePath, output, { spaces: 2 });
    await fs.writeJson(sidecarPath, {
      version: 1,
      template: agent.hooksTemplate,
      managedEvents,
      managedScripts: managedScriptNames,
    ...
    return { success: false, path: hooksFilePath, error: err.message };
  }
}
...
async function installToCanonical(templatesDir, canonicalDir) {
  ...
  // Antigravity CLI Plugin 清单（v6.7 起）：
  // - core/plugin.json → <canonical>/core/plugin.json
  // agy plugin install <canonical>/core 读取根级 plugin.json（Antigravity 的格式要求
  // 清单在插件根，而非 .claude-plugin/ 下）。不走 TEMPLATE_FILES，避免被 per-agent mount。
  const agyManifestSrc = path.join(packageRoot, 'plugin.json');
  const agyManifestDest = path.join(canonicalPackageRoot, 'plugin.json');
  if (await fs.pathExists(agyManifestSrc)) {
    await fs.copy(agyManifestSrc, agyManifestDest, { overwrite: true });
    results.copied.push('plugin.json');
  }
...
async function linkToAgents(sourceRoot, agentNames, options = {}) {
  ...
  for (const agentName of agentNames) {
    // Plugin 机制管理的 agent（claude-code / qoder）不走 installer。
    // 上层 CLI（bin/agent-workflow.js）已经 partition 过滤掉它们，
    // 这里加防御性跳过是避免其他调用者（如测试、postinstall）误传。
    if (agents[agentName] && agents[agentName].managedViaPlugin) {
      results[agentName] = {
        success: false,
        error: `${agentName} 由 Plugin 分发管理，请通过对应 plugin 模块调用`,
      };
      continue;
    }
    ...
    // 仅声明 agentsDir 的工具（如 Qoder）返回非 null → 才 mount subagents；其余工具跳过。
    const agentSubagentsDir = getAgentSubagentsDir(agentName, global, cwd);
    ...
    const agentResult = {
      ...
      subagents: { rootMode: 'directory', entries: {}, removed: [], count: 0 },
      hooksConfig: null,
    };

    // 5 个 per-agent 操作目标 disjoint 目录（commands / .agent-workflow/{hooks,specs,utils} / skills / agents / hooks-file）。
    // 并行启动节省 ~50ms/agent。subagents 仅对声明 agentsDir 的工具执行。
    const hooksFilePath = getAgentHooksFile(agentName, global, cwd);
    const [commandsResult, managedDirsResult, skillsResult, subagentsResult, hooksConfigResult] = await Promise.all([
      linkCommandEntries(sourceRoot, agentCommandNamespaceDir, fallbackToCopy),
      linkManagedDirs(sourceRoot, agentManagedDir, fallbackToCopy),
      linkSkillEntries(skillSourceRoot, canonicalDir, agentSkillsDir, fallbackToCopy),
      agentSubagentsDir
        ? linkAgentEntries(sourceRoot, agentSubagentsDir, fallbackToCopy)
        : Promise.resolve(null),
      hooksFilePath
        ? installAgentHooksConfig(sourceRoot, agentName, agentManagedDir, hooksFilePath)
        : Promise.resolve(null),
    ]);
    agentResult.commands = commandsResult;
    agentResult.managedDirs = managedDirsResult;
    agentResult.skills = skillsResult;
    if (subagentsResult) agentResult.subagents = subagentsResult;
    agentResult.hooksConfig = hooksConfigResult;
    if (hooksConfigResult && hooksConfigResult.success === false) {
    ...
    for (const linkResult of Object.values(agentResult.subagents.entries)) {
      if (!linkResult.success) {
        agentResult.success = false;
      }
    }
    ...
    agents: Object.fromEntries(
      Object.entries(result.agents).map(([name, agentResult]) => [
        name,
        {
          ...
          hooksConfig: agentResult.hooksConfig
            ? {
                installed: agentResult.hooksConfig.success !== false,
                skipped: agentResult.hooksConfig.skipped || false,
                path: agentResult.hooksConfig.path || null,
                sidecar: agentResult.hooksConfig.sidecar || null,
                template: agentResult.hooksConfig.template || null,
                managedEvents: agentResult.hooksConfig.managedEvents || [],
                backedUp: agentResult.hooksConfig.backedUp || null,
                error: agentResult.hooksConfig.error || null,
              }
            : null,
