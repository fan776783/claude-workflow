...
>
> 两类产物分工:机器 task 源(task-dir 元数据 + per-task `context.jsonl` 背包)由**本 skill 现写**(经 CLI,按 implementation slice 定粒度);可选的人类可读 plan.md 叙述骨架(front matter + 锚点)由 `/workflow-spec` Step 6 `spec-review` 生成,本 skill 仅在其上 Edit 扩写、不重建。
...
- 按真实实现切片确定 task 数量、ID、阶段、依赖、验收项,写入各 task 的 task.json(经 CLI,不手编)。spec-approve 落的壳可增删/合并/重排——不受预锁 task ID 约束。
...
为每个 task 策展 `context.jsonl` 文件指针背包(经 CLI 写,不手编),每行 `{file,reason}`:

- **仅 spec/research 路径**(spec.md / 调研笔记 / PRD 等),**禁 code 路径**(.js/.ts 等源码)——code 复用面走 contract-digest,源码 implementer 执行期自读。命中 code 路径执行期会被 `pre-execute-inject` 跳过 + stderr warn。
...
**输出**:现写 task-dir 元数据 + 各 task context.jsonl 背包(经 CLI);可选在 plan 叙述骨架上 Edit 扩写(路径由 `state.plan_file` 指定)。骨架模板:[`../../specs/workflow-templates/plan-template.md`](../../specs/workflow-templates/plan-template.md)。
