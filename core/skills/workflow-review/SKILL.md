...
记录前先标注本次执行模式（display label，对应 runtime 的 `single_reviewer` / `codex_enhanced`）:
```
📋 Review mode: single_reviewer | codex_enhanced | degraded-inline
```

- `single_reviewer`:Stage 1 主任务 + Stage 2 子 Agent（默认）
- `codex_enhanced`:Stage 1 主任务 + Stage 2 Codex + 子 Agent 并行
- `degraded-inline`:两 Stage 均会话内执行（子 Agent 不可用时降级）

降级时括号注明,例如 `codex_enhanced (codex_degraded)` / `codex_enhanced (agent_degraded)`。
...
`workflow-execute` 的并行批次需要 stage2 review 时(`scope: batch`),走 `workflow_cli.js advance --batch`（成功路径调用 `buildBatchPassGateResult`）或 `advance --batch-fail`（失败路径调用 `failWritableBatch`），**不经过本 skill**。

| 维度 | scope: workflow(本 skill) | scope: batch | scope: task |
|------|------|------|------|
| 触发 | 用户手动 `/workflow-review` | 批次合流后自动 | 命中 quality gate 的任务完成后（`nextTask.quality_gate` 为真 或 `actions` 含 `quality_review`） |
...
> scope: batch 入口在 `workflow_cli.js advance --batch`（成功）/ `advance --batch-fail`（失败）;scope: task 在 execute Step 5 由 `nextTask.quality_gate` 或 `actions` 含 `quality_review` 触发。未命中 gate 的普通任务完成后不走 scope: task review,在 workflow 全部完成时由 scope: workflow 统一覆盖。
