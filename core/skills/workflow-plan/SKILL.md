...
- task 粒度/ID 本阶段现写定型(spec-approve 只落壳);**不得手工修 state**——`task-write` 自动重导锚点,修锚走 `repair-anchor`(机制见 Hard Gate #3)
...
- **WorkflowTaskV2 Compatible** — task 以 task.json v2 经 `task-write` 落盘（字段见上「现写阶段」），非 plan.md markdown block
...
- **不要合太大**：一个 task 超过 2-5 分钟原子操作时拆分——否则 reviewer 难以构造有效 refute，review 变形式主义
- 相关 `lints.atomicity` / `lints.shared_file.merge_candidates` 由 `plan-review` 自动标记，命中人工确认

### Confidence Score

`plan-review` 自动算 confidence;偏低读 `confidence.hints` 提升,Step 3 paste `summary.confidence`,`low` 时标注「建议 review」。
