...
**自然语言映射**:`继续/连续` → continuous、`下一阶段/单阶段` → phase、`重试` → retry
...
- `halted` (`halt_reason: 'failure'`) → 提示 `--retry`(任务需永久移除走 `/workflow-delta` 脱范围)
...
2. Implementer 返回 `NEEDS_CONTEXT` 时，主会话**调用 `AskUserQuestion`** 收集人工答复:
   ...
   - options:`proceed`(把答案塞回 prompt 重派 implementer) / `defer`(任务保留 pending,workflow 进入 paused)
3. **按选择分流**:
...
> 任务需**永久移除**(非暂停)→ `/workflow-delta` 脱范围(`delta init --type requirement` → `remove` action),不在执行内跳过。

**降级**:`AskUserQuestion` 不可用 → 打印 implementer 反问并显式停在 Step 4(不进 Step 5),输出 `HITL wait: 请回复 "proceed/defer" 后我再继续`,等下条用户消息分流。HARD-GATE #5 无例外。
...
不再支持 task-skip。某 task 需**永久移除**(过时 / 超范围 / 改由人工处理)→ 走 `/workflow-delta` 脱范围:`delta init --type requirement` 描述移除 → 生成 `{action:'remove', task_id}` → `delta apply` 从 plan.md 删除该 task 并留变更迹。HITL `defer` 仅暂停,不等于移除。
