...
- 想看完整两阶段审查何时触发：看“触发时机”
- 想看 Spec 合规审查：看“阶段 1”
- 想看代码质量审查：看“阶段 2”
- 想看轻量合规检查与完整审查的区别：看开头规则与 `../../workflow-executing/references/execution-checklist.md`

## 何时读取

- task 命中 `quality_review`
- plan 到达最后一个 task
- 需要核对两阶段审查循环与降级执行规则时
...
> 每连续完成 3 个常规 task 触发的是**轻量合规检查**，不是完整两阶段审查。详见 `../../workflow-executing/references/execution-checklist.md`。
...
5. 偏差列表（如有）写入 `workflow-state.json` 的 `quality_gates` 字段（旧字段 `execution_reviews` 迁移规则参见 `../../../../specs/workflow-runtime/state-machine.md` → `execution_reviews` 迁移策略）
