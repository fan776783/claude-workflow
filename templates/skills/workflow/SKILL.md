...
结构化开发工作流：需求分析 → 需求讨论 → UX 设计审批 → Spec 生成 → Plan 生成 → 执行 + 子 Agent 审查。
...
2. **Plan 实时更新**：每个 task 完成后，**立即**将 `plan.md` 中对应的 `- [ ]` 改为 `- [x] ✅`。**禁止在所有任务完成后批量回写**。
...
- **计划层**：`plan.md`
  ...
  - 使用 checkbox 语法 `- [ ]` 追踪进度
  - 生成后执行 Self-Review 确保 spec 覆盖
- **执行层**：代码产出
  - 按 plan 步骤逐步实现
  - 每个任务完成后由子 Agent 执行两阶段审查

## 执行治理

`workflow execute` 采用子 Agent 驱动的审查机制：

```
执行任务 → 编码实现 → Spec 合规审查（子 Agent）→ 代码质量审查（子 Agent）→ 下一任务
...
- **两阶段子 Agent 审查** — Spec 合规审查 + 代码质量审查，每个任务完成后执行
...
```
executeTask() → 验证铁律（Gate Function）→ Spec 合规审查（子 Agent）→ 代码质量审查（子 Agent）→ 更新状态
```
...
在需求讨论（Phase 0.2）之后、Spec 生成（Phase 1）之前的设计审批阶段。**仅在检测到前端/GUI 相关需求时触发**：
...
统一的规范文档生成阶段，在**单一文档**中完成：

- **需求范围判定** — 每条需求标记 `in_scope / out_of_scope / blocked`，编号 R-001 起
- **关键约束提取** — 字段名、条件分支、数量限制等硬约束直接写入 Constraints 章节
- **架构与模块设计** — 模块划分、数据模型、接口设计、技术选型
- **用户可见行为** — 正常流程、异常流程、边界行为
- **验收标准** — 每个模块的验收条件和测试策略
- **文件结构** — 新建、修改和测试文件清单
- **实施切片** — 按可渐进交付的切片组织
...
- **Bite-Sized Tasks** — 每步 2-5 分钟，使用 checkbox `- [ ]` 追踪
...
| shared-utils | [references/shared-utils.md](references/shared-utils.md) | 任务解析、上下文估算 |
