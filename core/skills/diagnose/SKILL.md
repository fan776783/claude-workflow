---
name: diagnose
description: "Use when user says 'diagnose this' / 'debug this' / '怎么定位' / 'reproduce 不出来', or reports a bug that throws/fails, or describes a perf regression. 产出根因 + 推荐修复方案,不直接写修复代码 — 修复交给 /fix-bug 消费。"
---
...
### Stuck 触发 oracle 二次意见

假设证伪迭代到 **第 2 轮仍不收敛**（全部 ranked 假设证伪、无新的可证伪假设可生成、或新假设仍解释不了观察到的症状）→ 标 `stuck_or_looping`，由主会话按 `core/specs/shared/codex-routing.md § Decision Table` 调 `collaborating-with-codex` `--oracle-review`：把已证伪假设清单 + 反馈循环 + 观察到的症状作为 context 喂给 oracle 拿 alternative POV，回 Phase 3 重排假设。调用 contract 见 `codex-routing.md § Invocation Contract`（TASK / CONTEXT / FILES / RISK_SIGNALS / NON_GOALS 必填）。Codex 不可用 → 主会话兜底重排，Phase 5 输出 `oracle_status: codex_degraded` + 一句话降级原因。Codex 调用不阻断 Phase 推进。
...
- oracle_consulted: <true | false>  # Phase 3 stuck 是否触发过 codex oracle
- oracle_status: <ok | codex_degraded | skipped>  # consulted 时 ok / codex 不可用 codex_degraded / 未触发 skipped
- oracle_insight: <若 consulted=true 且 status=ok,oracle 给的关键 POV 摘要;degraded 时填降级原因;否则省略>
```
...
- "证伪 2 轮全错还在自己原 list 里凑新假设" — `stuck_or_looping`,应当调 codex oracle 拿 alternative POV,不要继续闭门循环
