...
> Knowledge 沉淀不在本步骤内自动执行。如在本次实现中遇到值得沉淀的内容（例如修了隐含约束类 bug、发现未记录的项目模式、做了带 tradeoff 的技术决策、识别到 DRY 违反），完成工作流后使用 `/knowledge-update` 命令显式捕获；相关违规检查由 `workflow-review` Stage 1 的 `/knowledge-check` 硬卡口负责。这样设计是为了把"sink 写入"和"生产代码的主管线"解耦，避免执行期被次要写操作干扰。
