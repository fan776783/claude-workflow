...
完整定义 + 触发点见 `core/specs/shared/manual-intervention-reasons.md`。
...
所有 `[HARD-STOP:...]` 按 `core/specs/shared/hard-stop-templates.md` 的相应模板调用 `AskUserQuestion`：
- `CONFIRM-PLAN` / `CONFIRM-BATCH-REVIEW` / `CONFIRM-COMMIT` / `BATCH-REVIEW-BLOCKER` / `CROSS-CONFLICT` 主要用 T2 / T3 变体，本文件各 Phase 内指明 options 填充。
...
编排模板见 `references/analysis-and-planning.md`。展示后 `[HARD-STOP:CONFIRM-PLAN]`，按 `hard-stop-templates.md § T2` 变体调用 AskUserQuestion：
...
**流转**按 `core/specs/shared/status-readiness.md § 流转动作`：
...
失败处理按 `status-readiness.md § 失败处理`。
...
调用方式按 `core/specs/shared/status-readiness.md`。示例模板 `references/status-and-reporting.md`。
