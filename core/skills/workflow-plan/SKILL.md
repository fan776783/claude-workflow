...
3. task 源 = task-dir(`~/.claude/workflows/{pid}/tasks/{taskId}/`),非 plan.md 物理解析。plan 阶段**按 implementation slice 现写最终 task 粒度/ID**(spec-approve 不再锁死);但 `current_tasks[0]` = task 源 `firstTaskId()` 始终是 resume 起点,重排/重命名后必须保证它仍解析到一个存在的 task(否则打穿 resume 三元组)
...
- task 粒度/ID 在本阶段现写定型(spec-approve 只落了元数据壳);现写后 `current_tasks[0]` = task 源 `firstTaskId()` 仍须解析到存在的 task

### 现写阶段定 task 粒度(task-dir 作 task 源)

机器 task 源是 **task-dir**(`~/.claude/workflows/{pid}/tasks/{taskId}/{task.json,context.jsonl}`),不是 plan.md 物理解析。spec-approve 阶段**只落了 task 元数据壳,未锁死最终粒度**(松 Hard Gate #3)。本阶段按 implementation slice **现写最终 task 切分**(对齐"最后一刻定粒度"共识):

- 按真实实现切片确定 task 数量、ID、阶段、依赖、验收项,**组成 JSON 数组一次性 `task-write` 整集写入**(原子替换 + 自动清孤儿,不手编 task.json、不写 `.cjs`)。spec-approve 落的壳可增删/合并/重排——不受预锁 task ID 约束。
...
- 现写后保证 `current_tasks[0]` = task 源 `firstTaskId()` 解析到一个存在的 task(C-1 resume 起点不可断)。
- **plan.md 退化为可选的人类可读叙述**(类 Trellis prd.md):front matter + 结构锚点 + 可选叙述。它**不再是结构化机器 task 源**——execute 期 `execution_sequencer` 从 task-dir 读 task 序列,不解析 plan.md task block。叙述 plan.md 仅供人 review,可省。
...
**输出**:`task-write` 整集写 task-dir 元数据 + `context-curate` 逐 task 写 context.jsonl 背包;可选在 plan 叙述骨架上 Edit 扩写(路径由 `state.plan_file` 指定)。骨架模板:[`../../specs/workflow-templates/plan-template.md`](../../specs/workflow-templates/plan-template.md)。
...
返回 JSON:
- `ready: false` → 按 `lints.placeholder.hits` / `lints.task_schema.issues`(非法 id 目录 / task.json 不可解析 / status 越界 / `empty_task_source` 空源 → 用 `task-write` 写入/重写修正)/ `coverage.uncovered_ids` / `lints.anchor_integrity`(Phase B 后)逐项修复 → 重跑直到 `ready: true`
