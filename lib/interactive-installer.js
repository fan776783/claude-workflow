...
async function runInteractiveInstall(options = {}) {
  ...
  // 7. 执行安装 —— 按 agent 类型分叉：claude-code 走 Plugin，其他 8 工具走 installer
  const ccTargets = targetAgents.filter(a => a === 'claude-code');
  const otherTargets = targetAgents.filter(a => a !== 'claude-code');
  ...
  try {
    ...
    // 若只有 claude-code 被选中，installer 没被调用，需要手动确保 canonical 就绪
    if (ccTargets.length > 0 && otherTargets.length === 0) {
