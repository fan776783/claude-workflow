...
```bash
PYTHON_BIN="$(command -v python3 || command -v python || command -v py)" && "$PYTHON_BIN" scripts/codex_bridge.py --cd "/path/to/project" --PROMPT "Your task"
```

If `python3` is unavailable, fall back to `python`, then `py`.
...
# Initial task
"$PYTHON_BIN" scripts/codex_bridge.py --cd "/project" --PROMPT "Analyze auth in login.py"

# Continue with SESSION_ID
"$PYTHON_BIN" scripts/codex_bridge.py --cd "/project" --SESSION_ID "uuid-from-response" --PROMPT "Write unit tests for that"
```
...
```bash
PYTHON_BIN="$(command -v python3 || command -v python || command -v py)"
"$PYTHON_BIN" scripts/codex_bridge.py --cd "/project" --PROMPT "Generate unified diff to add logging"
```

**Debug with full trace:**
```bash
PYTHON_BIN="$(command -v python3 || command -v python || command -v py)"
"$PYTHON_BIN" scripts/codex_bridge.py --cd "/project" --PROMPT "Debug this error" --return-all-messages
```
