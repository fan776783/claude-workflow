...
async function runInteractiveInstall(options = {}) {
  ...
  if (detectedAgents.length === 0) {
    ...
    p.outro(pc.yellow('请先安装 Claude Code / Cursor / Codex / Gemini CLI / GitHub Copilot / OpenCode / Antigravity / Droid 中的任一个'));
    process.exit(0);
  }
  ...
  for (const agentName of targetAgents) {
    ...
    if (config.managedViaPlugin) {
      summaryLines.push(`    ${pc.dim('via Claude Code Plugin (自动调用 claude plugin ...)')}`);
      continue;
    }
  ...
  // 7. 执行安装 —— 按 agent 类型分叉：Plugin 类工具（claude-code / qoder）走各自
  // Plugin 模块，其余工具走 installer mount。
  ...
  try {
    ...
    return { installResult, ccResult, qoderResult };
  } catch (err) {
