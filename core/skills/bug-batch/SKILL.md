...
| Phase 5.5 | Phase 5 所有 Task 已达终态（`completed` 或 `manual_intervention`） |
...
Layer N 的 Task：所有 `blockedBy` Task completed 后，推进为 `in_progress` 并启动子 agent。
...
所有 Task 达到终态后，对 `status_transition_ready = true` 且验证通过的 FixUnit 执行物化：
...
## Phase 5.5: 分层 codex review

`[PHASE:5.5/REVIEW]`

进入前置条件：Phase 5 所有 Task 已达终态，结果物化已完成。

### 5.5.1 单元级 review

在主会话收集到每个 FixUnit 的结果后，对 `status_transition_ready = true` 的单元逐个执行 review：
