...
| XL     | 架构变更、新子系统   | 建议切换 `/workflow start` |
...
2. **修改** → 回到 Step 3 调整
3. **切换到 /workflow start** → 用户明确需要完整管线
...
| 5-7  | 基本清晰，可能有少量不确定性                 |
| 1-4  | 不确定性较大，**建议切换到 /workflow start** |

## 与 workflow 的关系

- `/quick-plan` 是轻量规划入口：只产出 `plan.md`，不进入状态机。
- `/workflow start` 是完整规划入口：会生成 `spec.md`、`plan.md` 并写入 workflow 状态。
- 若 `/quick-plan` 在分析过程中发现任务升级为跨模块/高不确定性场景，应改走 `/workflow start`。
...
- 如果 plan 产出后用户希望走正式流程，可以 `/workflow start` 重新启动
