...
> 定义 `diff-review` 在 Quick / Deep 模式下共享的增强型报告结构。目标是在保持现有 Summary / Findings / Verdict 阅读习惯的同时，引入 verification 与 impact-aware 字段。
...
4. 允许 Deep 模式保留来源归属（Codex / Claude / Both）与统计信息

## 顶层结构

Quick / Deep 报告都应保留以下骨架：
...
Deep 模式可将标题渲染为 `# Deep Review Report`，但字段语义应一致。
...
| Field | Required | 说明 |
|------|----------|------|
| Review Mode | yes | `Quick` / `Deep` |
| Review Subject | yes | `HEAD` / `--staged` / `<base>...HEAD` |
...
如果 Quick 模式想保持简洁，也可统一保留 `## Findings`，但 P0/P1 需排在前面。
...
- 外部模型失败且无 P0 → `CORRECT (degraded)` 可在 Summary 的解释中说明
