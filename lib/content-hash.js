...
class StablePathSync {
   ...
   * @param {string} options.mode 同步模式（SyncMode 枚举值）
   * @param {string} options.planId 关联的计划 ID
   * @param {number} options.versionId 关联的版本 ID
   */
  constructor(options = {}) {
   ...
   * @returns {object} 包含 planId、versionId、contentHash、syncedAt 的元数据
   */
  buildMeta(content) {
    return {
  ...
  parseComment(raw) {
    const match = raw.match(/([\s\S]*?)<!-- agent-workflow-sync\n([\s\S]*?)-->\s*$/);
    if (!match) {
      return { content: raw, meta: null };
    }
