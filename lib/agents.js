...
const agents = {
  ...
  qoder: {
    name: 'qoder',
    displayName: 'Qoder',
    // Qoder CLI 支持 Claude Code 同款 Plugin 机制（lib/qoder-plugin.js），
    // commands/skills/agents/hooks 全部原生加载，通过 `qodercli plugins install`
    // 分发，不走 installer mount。skillsDir/globalSkillsDir 已移除；
    // detectInstalled 仍保留，供上层感知用户装了 Qoder。
    managedViaPlugin: true,
    detectInstalled: () => {
