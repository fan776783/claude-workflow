...
async function selectAgentsInteractive(options = {}) {
  ...
  const agentChoices = allAgents.map(name => {
    ...
    const pluginManaged = config.managedViaPlugin === true;
    let hint;
    if (pluginManaged) {
      hint = pc.yellow('via Claude Code Plugin');
    } else {
  ...
  // 默认预选检测到的 Agent；不再默认预选 claude-code（Plugin 机制由用户显式触发）
...
async function runInteractiveStatus(options = {}) {
  ...
  if (pluginStatus.installed) {
    ...
    lines.push(`  ${pc.red('✗')} 未安装 - 运行 agent-workflow sync -a claude-code`);
  }
