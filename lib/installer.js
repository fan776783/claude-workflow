...
async function getCanonicalSkillEntries(canonicalDir) {
  return getSkillEntries(canonicalDir);
}
...
async function linkNonSkillDirs(sourceRoot, agentBaseDir, fallbackToCopy = true) {
  const links = {};
...
async function linkToAgents(sourceRoot, agentNames, options = {}) {
  ...
  for (const agentName of agentNames) {
    ...
    agentResult.links = await linkNonSkillDirs(sourceRoot, agentBaseDir, fallbackToCopy);
...
async function ensureWorktreeHooks(settingsPath, hooksDir) {
  ...
  for (const [eventName, hookDef] of Object.entries(WORKTREE_HOOK_DEFS)) {
    ...
    // 构建 hook command（使用绝对路径）
    // 注意：不能使用 ~ 缩写，因为 node "~/.claude/..." 中 ~ 在双引号内不会被 shell 展开
    const commandPath = scriptPath;
...
async function linkRepoToAgents(options = {}) {
  ...
  const canonicalDir = getCanonicalDir(global, cwd);
  const sourceRoot = path.resolve(templatesDir);
  const result = {
    canonicalDir,
    agents: {},
    errors: [],
...
function detectAgentMode(agentStatus, installMode, sourceRoot) {
  ...
  if (installMode === INSTALL_MODE_REPO_LINK) {
    const repoLinked = [
      ...
      ...Object.values(agentStatus.nonSkillDirs),
  ...
  if (Object.values(agentStatus.skills).some(skill => skill.mode === 'copy')) {
    return 'mixed';
  }
...
async function getInstallationStatus(global = true, cwd = process.cwd()) {
  ...
  if (await fs.pathExists(metaFile)) {
    try {
      ...
      status.sourceRoot = meta.sourceRoot || canonicalDir;
    } catch {}
  }

  const expectedRoot = status.mode === INSTALL_MODE_REPO_LINK
    ? status.sourceRoot
    : canonicalDir;
