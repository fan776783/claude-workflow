...
- **跨服务契约随引用自动入背包**:若 task 的 spec / slice 引用了契约锚点 `<file>#anchor`(如 `docs/contracts/*.md#seam-*`、`docs/architecture/README.md#seam-domain-*`),把锚点 `#` 前的 `.md` 路径 curate 进该 task 的 `context.jsonl`(`reason` 写引用的 seam id)。execute 期经 `<context-pack>` 注入,让 implementer 看到**权威契约原文**而非 spec 里的二手转述。路径按引用自身解析,项目无关;契约是 SSOT,只引用不在 task 里重述字段。
