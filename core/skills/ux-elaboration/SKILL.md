...
> `workflow-spec` Step 5 委托 ux-elaboration 时**不传**设计源参数——PRD 中的图片/URL 不一定是布局参考，需要人在 Step 4 显式策展。
...
AI 归一化后**始终展示匹配结果给用户复核**：回 `1`/`OK` 继续，或"PageA 换成 img2"这种重映射（需带内容，不编号）。
...
补充 PageC 的设计源（需带内容），或回 1 / "跳过 PageC" 跳过该页（标 `infer`）。
```
