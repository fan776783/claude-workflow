...
- **writable fan-out**（对齐 superpowers）：独立写任务，前提是**文件不重叠 + 无共享状态**。每 agent 改自己 scope 内的文件，主会话回收后做 conflict check + 跑全量验证。
...
# writable fan-out（额外）
files_changed: [<本 agent 改过的文件，用于主会话 conflict check>]
...
3. **writable fan-out 必做**（对齐 superpowers Review & Integrate）：
   - 用各 agent 的 `files_changed` 做 **conflict check** —— 出现交集即视为前置违规，人工核对
...
- writable fan-out 回收后 `files_changed` 出现交集 → 视为独立性误判，**回退到顺序重做**该组，不强行 merge
