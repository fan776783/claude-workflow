...
展示 FixUnit 编排结果并请求确认时，读取 `references/analysis-and-planning.md` 中的模板。在收到用户确认前不得继续进入修复。
...
输出 `[HARD-STOP:CONFIRM-BATCH-REVIEW]`：
...
## 是否启动 codex 对协调分支整体执行批量 adversarial review？(Y/N)
```

用户回复规则：仅接受明确 `Y` 或 `N`；无回复时**停止等待**，不得自动按建议执行；用户显式"本批次后续全部 Y / 全部 N"可作为批量指令沿用到后续条件性 Hard Stop。
...
按用户回复分支：
- `Y` → 启动 codex adversarial review
- `N` → 主会话直接审查协调分支整体 diff，记录 `batch_review_mode: "self"` 后继续
- 无回复 → 停在 Hard Stop
...
- **阻断性问题**：触发 `[HARD-STOP:BATCH-REVIEW-BLOCKER]`，列出以下选项：
  1. 继续推进（忽略警告，进入 Phase 6；仅在用户明确判定为误报时选择）
  2. 仅剔除受影响单元（先执行失败级联；再把 review 点名的 FixUnit 标记 `cross_unit_conflict`；受影响单元物化代码保留在协调分支，用户后续人工决定回滚与否）
  3. 放弃本批次全部已物化修改（先对每个作为 `covered_by_unit` 的已物化单元执行失败级联；再回滚协调分支上所有已物化 FixUnit，缺陷状态从"处理中"回退到"待处理"；进入 Phase 8）
...
若用户对某个 FixUnit 不认可，询问处理方式（Hard Stop 内二次确认）：

```markdown
FU-<unit_id> 不认可，请选择处理方式：
1. 放弃修改：该单元代码不进入最终重建分支，缺陷状态回退到"待处理"
2. 保留代码但不提交：该单元代码不进入最终重建分支，缺陷保持"处理中"
3. 仅从本次 Commit 移除：同选项 2，但在汇总中标注"用户计划另行处理"
```
...
**依赖冲突**：被拒绝 FixUnit 与某个已确认 FixUnit 存在依赖关系（后者建立在前者基础上）时，cherry-pick 重建会冲突。触发 `[HARD-STOP:DEPENDENCY-CONFLICT]`，列出依赖对，让用户决定：一并拒绝依赖方、将依赖方降级为 manual_intervention、或放弃拒绝恢复原确认。
