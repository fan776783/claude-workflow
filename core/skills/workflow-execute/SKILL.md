...
返回 JSON `{fresh, content?, reason?, fallback?}`。`fresh:true` → 用 handoff 里的 Decisions/Rejected/Risks + contract-digest 指针辅助 Step 4/5 派发;`fresh:false`(stale/missing)→ 不阻断,回退按既有 state-first 路径读 plan.md(经 task-bundle 切片)。
...
| ② 落 quality_gate | **必选** | reviewer PASS 后 `quality_review.js pass {taskId}` 落精简 per-task review record（CLI 写盘，不回灌 controller） |
| ③ Checkpoint | **必选** | `advance {taskId}` 更新 plan.md + state.json |
| ④ Journal | 暂停/workflow 完成时 | `workflow_cli.js journal add` |

> Step 5.2 reviewer PASS 且 Step 6.① 验证通过后才能落 gate + advance。
...
### ② 落 per-task quality_gate（reviewer PASS 后，必做）

Step 5.2 reviewer 终判 `PASS`(critical/important 必为 0) 且 ① 验证通过后,把 reviewer verdict 落一条精简 per-task gate record。**CLI 写盘,不把 `gate_result` 全文回灌 controller**——只读返回的 `overall_passed` 确认(守 ADR 0002 "reviewer 输出不污染 controller 上下文")。

```bash
node ~/.agents/agent-workflow/core/utils/workflow/quality_review.js pass {taskId} \
  --project-id {projectId} \
  --from-task {taskId} --to-task {taskId} \
  --files-changed {n} --requirement-ids {task 需求 ID} \
  --stage1-attempts {implementer↔reviewer loop 次数} --stage2-attempts 1 \
  --minor-count {Phase 2 minor 数} \
  --review-mode single_reviewer --reviewer {subagent|self}
```

- `--reviewer subagent`(claude-code/cursor/codex)/ `self`(无 subagent 平台 self-review)。`--base-commit` / `--current-commit` 省略 → CLI 自解析 `initial_head_commit` + git HEAD。
- 这条 record 是 `workflow-review` Step 0.2 / HG-5 / Stage 2 §3.1 的 per-task review 审计锚点;缺它则 review 阶段判 per-task trace 缺失。
- 与 `task.quality_gate` bool(commit 边界 marker,ADR 0002)是两回事:本步写 `state.quality_gates[taskId]` review record。
- reviewer 终态 FAIL 已由 Step 5.2 loop 上限 → halt 处理;如需审计可 `quality_review.js fail`,非必须。
...
3. **写 handoff(execute→review)**:把执行阶段决策蒸馏成 handoff 交给 review——正文 ≤20 行(CLI 自动拼 5 行 freshness header),建议 `## Decisions`(实现偏离 spec 处+理由)/ `## Rejected`(放弃的实现路径)/ `## Risks`(待 review 重点核对的跨 task contract)+ contract-digest 指针。不复写 report 正文。同 set-report-path 一样走 CLI 落盘:
