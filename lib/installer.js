...
async function linkToAgents(sourceRoot, agentNames, options = {}) {
  ...
  for (const agentName of agentNames) {
    ...
    if (agentName === 'claude-code') {
      ...
      // Subagent 文件同步：从 core/agents/ 静态复制
      try {
        agentResult.agentFiles = await syncAgentFiles(sourceRoot, agentBaseDir);
      ...
      if (global) {
        try {
          ...
          agentResult.workflowHooks = await ensureWorkflowHooks(settingsPath, hooksDir, {
            enableStrict: Boolean(enableWorkflowHooks),
          });
        } catch (err) {
        ...
        agentResult.workflowHooks = {
          ...
          base: {
            injected: false,
            events: [],
            skipped: ['project-level install skips base workflow hook injection'],
          },
          strict: {
            injected: false,
            events: [],
            skipped: ['project-level install skips strict workflow hook injection'],
          },
        };
      }
    }
...
const WORKFLOW_STRICT_HOOK_DEFS = {
  PostToolUse: {
...
async function ensureWorkflowHooks(settingsPath, hooksDir, options = {}) {
  const { enableStrict = false } = options;
  const base = await ensureManagedHooks(settingsPath, hooksDir, WORKFLOW_BASE_HOOK_DEFS);
  const strict = enableStrict
    ? await ensureManagedHooks(settingsPath, hooksDir, WORKFLOW_STRICT_HOOK_DEFS)
    : {
        injected: false,
        events: [],
        skipped: ['strict workflow hooks disabled; use sync/link --workflow-hooks to register PostToolUse'],
      };

  const result = {
    injected: Boolean(base.injected || strict.injected),
    events: [...(base.events || []), ...(strict.events || [])],
    skipped: [...(base.skipped || []), ...(strict.skipped || [])],
    base,
    strict,
  };
      ...
      Object.entries(result.agents).map(([name, agentResult]) => [
        name,
        {
          ...
          agentFiles: name === 'claude-code' && agentResult.agentFiles ? {
            ...
            path: path.join(getAgentBaseDir(name, global, cwd), AGENTS_DIR),
          } : undefined,
        },
      ])
    ),
  ...
  if (await fs.pathExists(canonicalDir) || (status.mode === INSTALL_MODE_REPO_LINK && await fs.pathExists(status.sourceRoot))) {
    ...
    for (const [name, config] of Object.entries(agents)) {
      ...
      if (name === 'claude-code') {
        ...
        agentStatus.workflowHooks = {
          base: await inspectManagedHooks(settingsPath, hooksDir, WORKFLOW_BASE_HOOK_DEFS),
          strict: await inspectManagedHooks(settingsPath, hooksDir, WORKFLOW_STRICT_HOOK_DEFS),
        };

        // Subagent 文件状态检查
        const agentFilesDir = path.join(getAgentBaseDir(name, global, cwd), AGENTS_DIR);
