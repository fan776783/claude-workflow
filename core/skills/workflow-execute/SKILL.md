...
- `halted` (`halt_reason: 'dependency'`) → 提示 `workflow_cli.js unblock <dep>`
...
`workflow-state.json` 不存在 → `workflow_cli.js init` 自动从 plan.md 推导首个未完成任务并创建最小状态文件。多个 plan 候选时 CLI 返回错误（`detected_plans` 数组）→ 展示列表让用户选择,不自行猜测。
...
| `pause-governance` | 因治理 phase 边界暂停（state → `halted`, `halt_reason: 'governance'`） |
...
- `batch_orchestrator.js config` 返回配置对象中 `enabled: false` 或 `maxConcurrency <= 1` → 跳过并行
...
`mode: progressive` 时:自动跳过被阻塞任务(`blocked_by` 依赖未解除)→ 只执行可执行任务 → 所有任务被阻塞时转为 `halted`（`halt_reason: 'dependency'`）→ 用户 `workflow_cli.js unblock <dep>` 解除后继续。
