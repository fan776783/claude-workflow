...
const TEMPLATE_DIRS = ['commands', 'agents', 'utils', 'prompts', 'skills', 'specs', 'project'];
...
async function copyTemplatesToClaude({ templatesDir, claudeDir, overwrite = true }) {
  ...
  for (const name of TEMPLATE_DIRS) {
    ...
    if (await fs.pathExists(src)) {
      ...
      // Use recursive copy to handle nested directories like prompts/codex/
    ...
    for (const entry of entries) {
      ...
      if (!destExists) {
        ...
        if (originalContent && destContent === originalContent) {
          ...
          mergeLog.push({ file: `${dirName}/${relPath}`, action: 'CONFLICT', newFile: `${path.basename(destPath)}.new` });
        }
      }
    }
  }
