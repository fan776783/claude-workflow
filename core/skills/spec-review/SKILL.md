---
name: spec-review
description: "审查 code-specs 库（v2.2 对齐 Trellis）。按文件类型分档 lint：convention 查必备 4 段 + 代码示例 + Why；contract 查 7 段；通用维度查过期/冲突。模板漂移走 .template-hashes.json。"
---

# /spec-review

只读的 code-specs 库审查命令。对齐 Trellis 的声明式审查模型：按文件类型分档 lint + 过期 + 冲突 + 模板漂移对账，输出报告供用户决定后续动作。
...
- 对齐 Trellis 核心原则"2-3 real code examples per guideline"

**v2.2 新维度 no-rationale**：

- Rules 段内**无 `**Why**:` 行**（也接受 `Why：` / `原因：` 变体） → `⚠️ no-rationale`
- 对齐 Trellis 核心原则"每条规则要说明 Why"
...
- 对齐 Trellis 渐进填充理念：有空 layer 不是问题，只是提醒
...
- **不**把覆盖率（filled/total）作为核心指标（对齐 Trellis 渐进填充）
- 空 layer 改 advisory（对齐 Trellis live 的渐进理念）
- 新增 no-examples / no-rationale 两个维度，对齐 Trellis "2-3 real examples + Why" 硬标准
