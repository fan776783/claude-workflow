...
2. **每个 agent 的字段**必须非空：
   ...
   **例外 —— `managedViaPlugin` agents**：`claude-code`、`qoder` 走各自工具的原生 Plugin 机制分发（`lib/claude-code-plugin.js` / `lib/qoder-plugin.js`，通过 `claude plugin install` / `qodercli plugins install`），不经过 installer mount。它们只需 `name`、`displayName`、`detectInstalled`、`managedViaPlugin: true`，**豁免** `skillsDir` / `globalSkillsDir`。validator 自动按 `managedViaPlugin` 跳过这两项。
3. **skillsDir convention**（仅 mount-agents 适用）：必须是 `.<agent-home>/skills` 形式，目的地由 `getAgentBaseDir()` 反推得到。允许 `.agent/skills`、`.claude/skills` 等现有convention；禁止指向 `skills/` 根目录（否则会与 canonical 冲突）。
...
- 任一 mount-agent 缺少上述 5 个必填字段（`managedViaPlugin` agents 豁免 skillsDir/globalSkillsDir）
