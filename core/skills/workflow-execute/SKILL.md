...
删 governor 后无可靠 in-session token 计数,不做计数判断(实测单会话上限约 5–7 task)。长 run 后感知 context 偏满时,打一行 banner 建议用户本批结束后 `/clear` 开新会话续跑——resume 三元组从 state + task-dir 重建(见 Step 2),清理无损失。**不阻塞、不写盘**,用户可忽略。
