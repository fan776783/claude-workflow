...
- **Alignment & architecture** — `grill` (interview-until-alignment, replaces `enhance`), `tdd` (red-green-refactor discipline), `write-a-skill` (meta-skill for creating new skills).
...
- **Navigation & git** — `ask-workflow` (skill 路由地图,不知道用哪个 skill 时看,纯导航不干活,`disable-model-invocation`), `resolve-merge-conflicts` (逐 hunk 解决 git merge/rebase/cherry-pick 冲突,保留双方意图,永不 `--abort`).
